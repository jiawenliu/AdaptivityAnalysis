%% \highlight{We illustrate here how our analysis work on an example.
%% We consider the Algorithm $\kw{multipleRounds}$, a simplified form of the \emph{monitor argument} by \citet{RogersRSSTW20}. 
...
%% here for the sake of space limit.

$\THESYSTEM$ does not  provide tight upper bounds on the adaptivity, when this has a path-sensitive nature. We show this on one example, $\kw{multiRoundsOdd(k)}$, presented in Fig.~\ref{fig:multiRoundsOdd}(a). %% For this program, $\THESYSTEM$ over-approximate the adaptivity.
