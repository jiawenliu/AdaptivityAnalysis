...
To define dependency between program variables we will consider two events that are generated from the same command, hence they have the same variable name or boolean expression and label, but have either different value or different query expression.
\highlight{we use $\eventset$ to denote the set of all events, $\cdom$ for a set of all the programs.} This is captured by the following definition. 
...
\end{defn}
It is worth stressing that in the definition above, if two events are both generated from query requests, we are not comparing the query answers (the element in the third position of the tuple), but the query expressions (the element in the fourth position of the tuple).
...
\end{defn}
There are several components in this definition. The part with label (2a) requires that $\event_1$ and $\event_1'$ differ in their values ($\diff(\event_1, \event_1')$).
...
For the control dependency we check whether the change \highlight{of the value in the assignment of event} $\event_1$ affects the \highlight{occurrence} of the event $\event_2$ or not. 
...
We can now define the \emph{semantics-based dependency graph} of a program $c$. We want this graph to combine quantitative reachability information with dependency information. 
...
\highlight{In Fig.~\ref{fig:overview-example}(b), $\lambda \trace_0 \st (l^6 \to x^3)$ is a walk with two vertices and where each vertex is visited only once. 
With the assumption that $k \geq 1$, $\lambda \trace_0 \st (l^6 \to \underbrace{a^5 \to a^5 \to \ldots}_{\env(\trace) k} \to x^3)$ is a walk where the vertex $a^5$ is visited $\env(\trace) k$ times.
