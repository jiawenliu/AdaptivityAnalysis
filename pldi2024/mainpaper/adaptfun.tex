...
The {\em feasible data-flow analysis} computes for every pair $x^i, y^j \in \lvar(c)$
 ...
 This analysis is based on a relation $\flowsto(x^i, y^j, c)$ built over the sets $\live(l, c)$ for every location $l$, whose definiton of is in the supplementary material.  
...
\begin{algorithm}
            ...
            \STATE \qquad \qquad \qquad  $\kw{visited}[v] = {true}$; %\#\{mark $v$ as visited\}
