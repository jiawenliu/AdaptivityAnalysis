...
construct an abstract transition graph \highlight{ $\absG(c)$ of a program $c$, a
graph with the set of labels of program points in $c$  (including a 
label $\lex$ for the exit point) as the set of
vertices $\absV(c)$, and with the set of transitions in $c$ as the set of
edges $\absE(c)$. Each edge of the graph is annotated with either
the symbol $\top$, a boolean expression or a \emph{difference
constraint}~\cite{sinn2017complexity}. A difference constraint is an inequality of the form $x' \leq y + v$ {or $x' \leq v$}
where $x, y$ are variables and $v \in \constdom$ is a symbolic
constant: either a natural number, the symbol $\infty$, an input
variable or a symbol $Q_m$ representing a query request.  We denote by
$\dcdom$ the set of difference constraints. }
...
We denote the reachability-bound of an edge $e = (l, dc, l') \in \absE(c)$ as $\absclr(e, c)$. More details in the supplementary material.% Fig.~\ref{fig:abscfg_tworound}(c) has the bounds $\absclr(e, c)$ for each edge $e$. 
...
% \end{thm}
% }
Symbolic expressions as used in the weight are great to express symbolic bounds but make the \highlight{direct} computation of 
a maximal walk harder. Specifically, \highlight{to directly compute the estimated adaptivity $\progA$},
one has to face two challenges. The first is non-termination.
...
It would consistently and considerably over-approximate the adaptivity. \highlight{The challenges 
of the direct compuation of $\progA$ makes us develop a $\pathsearch$ algorithm that provides a sound upper bound 
on $\progA$. }
...
complexity is O(V+E) where $V$ and $E$ represent the number of vertices and the number of edges 
...
we need to approximate an upper bound on the query length of the walk with the maximal query length in this SCC. 
Suppose we have an SCC with just three vertices $a^{3}_{1}$, $b^{2}_{1}$,
$c^{3}_{0}$, $a$ and $c$ are query vertices with weight $3$ and $b$ is a standard assignment vertex with weight $2$. 
This SCC is a directed cycle where $a \to b \to c \to a \dots$.  
The query length of the walk we are looking for, for instance, $a \to b \to c \to a \to b \to c \to a$, $5$, can be approximated by
 first finding such a path in this SCC
which starts from a vertex and stops when it reaches the same vertex, such as $a \to b \to c \to a$. Then the product of 
 the minimal weight along the path ($\min(3,2,3,3)$) and the number of queries of this path $3$ ($a$ and $c$ are query vertices), $6$,
 will be the sound upper bound we are looking for. Because we are
 ...
 stops when it goes back to itself (line: 11 in Algorithm~\ref{alg:adaptscc}). When a cycle is detected,
 ...
 and the number of query vertices along the visited path as we discussed in the above intuition. When all the traverses starting from 
 every vertex of this SCC are finished, the remaining upper bound will be the maximal approximated one. 
...
\begin{algorithm}
            ...
            \STATE \qquad \qquad \qquad  $\kw{visited}[v] = \highlight{true}$; %\#\{mark $v$ as visited\}
