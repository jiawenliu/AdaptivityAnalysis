...
\end{center} 
  ...
  The complete proof is in supplementary material Appendix~C. Below we show a proof sketch. This lemma is proved by Theorem~\ref{thm:flowsto_soundness}, which shows that for arbitrary two $x^i, y^j \in \lvar_c$, there exists a static ``$\flowsto$ chain'' from $x^i$ to $y^j$ when $\vardep(x^i, y^j, c)$.
  \begin{thm}[$\vardep$ implies $\flowsto$]
    \label{thm:flowsto_soundness}
    Given a program ${c}$, for all  $ x^i, y^j \in \lvar_{{c}}$, if $\vardep(x^i, y^j, {c})$,
    then
...
Because we are
 ...
 The traverse is naturally performed by a Depth First Search, whose complexity is O(V+E), where $V$ and $E$ 
...
and updates the weight when the path reaches a certain vertex, guarantees that every vertex on the estimated walk is allowed to be visited at most $\kw{flowcapacity[v]}$ times, and this walk is a valid finite walk.
%
