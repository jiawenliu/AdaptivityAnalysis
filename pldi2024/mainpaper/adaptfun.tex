...
$c$.  \remove{The high level idea behind {\THESYSTEM} is to first build
...
\highlight{ 
  ...
  \begin{enumerate}
    ...
    \item use the reachability-bound algorithm adapts to our setting inspired from previous work~\cite{ZulegerGSV11,SinnZV14,sinn2017complexity} 
    to find out the upper bound
    on how many times each node can be executed, as the weight for every node in the dependency graph;
    \item  use our algorithm AdaptBD to estimate an upper bound on, the number of queries in the walk over the weighted dependency graph that visits most query nodes 
    , which is adaptivity. In particualr, a walk is allowed to visit a node multiple times but not exceed the weight.
  \end{enumerate}
  The \emph{estimated dependency graph} overapproximates the
  semantics-based dependency graph in two dimensions: it
  overapproximates the dependencies between assigned variables (Section~\ref{sec:alg_edgegen}), and, it
  overapproximates the weights (Section~\ref{sec:alg_weightgen}). In this sense, 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\highlight{\subsection{Weighted Dependency Graph Construction}}
...
\subsubsection{\highlight{Edge Construction}}
\highlight{The set of edges $\progE(c)$ is constructed through a \textbf{feasible data-flow analysis}, 
a data flow analysis over 
our query language, combined with the reaching definition analysis.
We denote the results of reaching definition analysis, $\live(l, c)$ for a set of reachable 
variables at line $l$ with respect to the program $c$, which plays the role of removing redundent edges 
in data flow analysis. }

\highlight{\textbf{Feasible data-flow analysis} computes for every pair $x^i, y^j \in \lvar(c)$
...
\highlight{we can construct} an edge by computing a transitive closure (through other variables) of the  
...
\highlight{The weight of every vertex is obtained as an upper bound on 
...
Our reachability-bound analysis adapts previous work~\cite{ZulegerGSV11,SinnZV14,sinn2017complexity} 
to our query language setting. Similarly as these work, we first need to 
construct an abstraction transition graph in order to perform the 
reachability-bound analysis.}

\highlight{The \emph{Abstract Transition Graph} $\absG(c)$ of a program $c$ is a
graph with the set of labels of program points in $c$  (including a 
label $\lex$ for the exit point) as the set of
vertices $\absV(c)$, and with the set of transitions in $c$ as the set of
edges $\absE(c)$. Each edge of the graph is annotated with either
...
\highlight{We show in Fig.~\ref{fig:abscfg_tworound}(b) the  abstract control flow graph,
$\absG(\kw{twoRounds(k)})$ of the $\kw{twoRounds(k)}$ program we gave in 
 Fig.~\ref{fig:overview-example}(a) and which we also report in 
  Fig.~\ref{fig:abscfg_tworound}(a).}
...
the Abstract Transition Graph $\absG(c)$ of $c$ using \emph{reachability-bound analysis}~\cite{GulwaniZ10}.
 Specifically, we estimate as the weight of a node with label $l$ a symbolic upper bound on the 
...
\highlight{The set $\progW(c)$ of weights for the estimated dependency graph is estimated from 
the Abstract Transition Graph $\absG(c)$ of $c$ using our reachability-bound analysis.
 Specifically, we estimate as the weight of a node with label $l$ a symbolic upper bound on the 
 execution times of the command with label $l$ obtained by reachability-bound analysis. 
 These symbolic upper bounds are expressions with the input variables as free variables, 
 hence they correspond to the weight functions in the semantics-based dependency graphs. }

\highlight{When performing our reachablity-bound analysis on the abstract transition graph $\absG(c)$
of a program $c$, we donte the reachability-bound $\absclr(e, c)$ 
for every edge $e = (l, dc, l') \in \absE(c)$.} 
...
%
\subsubsection{\highlight{Formal Definition of Estimted Dependency Graph}}
\highlight{\begin{defn}
...
\highlight{where $\qlen$ and $\walks$ are defined in Section~\ref{sec:sematnic_adaptivity}.}
%
...
\highlight{We propose a program analysis algorithm $\pathsearch$ which 
...
\highlight{\subsection{$\pathsearch$ Algorithm}}
...
\begin{enumerate}
  ...
  its strongly connected components (SCCs) using the standard Kosaraju’s algorithm. SCCs are
...
To provide an upper bound on the local adaptivity of a Strong Connected Component of a graph,
we need to find out the walk with maximal query length in this SCC. 
Suppose we have an SCC with just three vertices $a^{3}_{1}$, $b^{2}_{1}$,
$c^{3}_{0}$, $a$ with weight $3$ and query annotation marked as $1$, similar for $b$ and $c$. 
The walk we are looking for, $a \to b \to c \to a \to b \to c \to a$, can be approximated by finding a path in this SCC
which starts from a vertex and stops when it reaches the same vertex, such as $a \to b \to c$, by the product of 
 the minimal weight along the path (min(3,2,3)) and the number of queries of this path 2 ($a$ and $c$ are query nodes). 
 Because we are
 ...
 as traverse for every vertex so the total complexity for $\pathsearch_{\kw{SCC}}$ is O((V+E)*V).
 }
