...
program.} 
\highlight{ 
  The high level idea behind {\THESYSTEM} is the following: 1. construct
  an \emph{estimated program dependency graph} \progG(c) of a program $c$
  (Section~\ref{sec:alg_weightedgegen}) which overapproximates the
  semantics-based dependency graph. 
  2. use reachability-bound algorithm adapts to our setting inspired from previous work~\cite{ZulegerGSV11,SinnZV14,sinn2017complexity} to find out the upper bound
  on how many times each node can be executed, as the weight for every node in the dependency graph. 3. use an algorithm
  AdaptBD to estimate an upper bound on, the number of queries in the walk over the weighted dependency graph that visits most nodes 
  associated with query label, which is adaptivity.  4. In Theorem\ref{thm:adaptalg_soundness}, we prove the upper bound provided by AdaptBD is a sound upper bound
  over the adaptivity in Definition~\ref{def:trace_adapt}.  
}
