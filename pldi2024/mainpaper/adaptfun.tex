...
\highlight{ 
  ...
  \begin{enumerate}
    ...
    on how many times each vertex can be executed, as the weight for every vertex in the dependency graph;
    \item  use our algorithm AdaptBD to estimate an upper bound on, the number of queries in the walk over the weighted dependency graph that visits most query vertices 
    , which is adaptivity. In particualr, a walk is allowed to visit a vertex multiple times but not exceed the weight.
  \end{enumerate}
