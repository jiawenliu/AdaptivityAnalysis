...
\end{center} 
  ...
  Lemma~\ref{lem:edge_map} is proved by Theorem~\ref{thm:flowsto_soundness}, which shows that for arbitrary two $x^i, y^j \in \lvar_c$, there exists a static "$\flowsto$ chain" from $x^i$ to $y^j$ when $\vardep(x^i, y^j, c)$.
  \begin{thm}[$\vardep$ implies $\flowsto$]
    \label{thm:flowsto_soundness}
    Given a program ${c}$, for all  $ x^i, y^j \in \lvar_{{c}}$, if $\vardep(x^i, y^j, {c})$,
    then
    ...
    \]
    \end{thm}
    In summary, for arbitrary two $x^i, y^j \in \lvar_c$ with \emph{Variable May-Dependency} relation, in order to show they have 
    ...
    $$
%
These two cases are then proved by the following two lemmas:
\\
...
2. The middle event with a sub-trace implies a "sub $\flowsto$-chain", 
 by induction on the trace $\trace$ and the Theorem~\ref{thm:flowsto_event_soundness}
