...
to $5104$ seconds ($85$ minutes) for the $21^{th}$ example $\kw{mRCom}$ who has two $3-$level nest while loops. The more while loops the examples contain, the higher level of these loops 
 the much longer time neeeded for weight estimation.
Notice, even though the example $\kw{seqCom}$ has the largest size of $502$ lines 
of code, our tool can still quickly handle it because $\kw{seqCom}$ does not have loops, the time is only $1.27$ seconds
for the weight estimation. Fortunately, examples with so many multiple loops
...
components of {\THESYSTEM}. \remove{For this, we implement three alternative
...
\highlight{To this end, we present three alternative
...
w.r.t. the true value. The first one {\THESYSTEM}-I replaces the reachability bound analysis algorithm in Section~\ref{sec:alg_weightgen}
with a light one that handles nested while loop effectively but coarsely so it can quickly take care of examples $\kw{jumbo}$ and $\kw{big}$.
...
Thresholdout mechanism if these two quantities are close. \highlight{We mark the best rmse in red.
It shows that in most cases, the choice of mechanism guided by our heuristics is the best one. 
}
 It is worth
...
\highlight{
    ...
    the instantiation of input parameters such as $k,m, n$. To better present the 
    robustness of our heuristics, we present two different instantiatons $k,m,n=10$ and $k,m, n =1000$ so that
    ...
    Table~\ref{tb:adapt-generalization}.
}
...
\highlight{The $2^{nd}$ to $5^{th}$ programs represent different algorithms for fitting a two-dimensional data set of size $n$ into a linear relation.
...
We have four data analyses \remove{which are the implementation of algorithms} from~\cite{Jamieson2015TheAO} within the \emph{Guess and Check}~\cite{RogersRSSTW20} framework.
% is the
