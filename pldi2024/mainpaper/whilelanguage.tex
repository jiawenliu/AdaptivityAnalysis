...
Each command is annotated with a label $l$, we will use natural numbers as labels to record
the location of each command, so that we can uniquely identify them.
We also have a set $\mathcal{LV}$  of labeled variables (simply variables with a label), a set $\cdom$ of all the programs.
...
and we use $\eventset^{\asn}$ and $\eventset^{\test}$ to denote the set of all assignment events (in blue) and testing events(in red), respectively.
\begin{center}
$ \begin{array}{lllll}
\mbox{Event} 
& \event & ::= & 
\textcolor{blue}{({x}, l, v, \bullet)} ~|~ \textcolor{blue}{ ({x}, l, v, \qval)}  ~|~\textcolor{red}{(\bexpr, l, v, \bullet)}  
\\
\end{array}$
\end{center}
