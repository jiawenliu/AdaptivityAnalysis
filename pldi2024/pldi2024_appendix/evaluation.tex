...
because of the path insensitive nature of the weight estimation algorithm. 
...
$\THESYSTEM$.
For the $17^{th}$ example ($\kw{loop2R}$), {\THESYSTEM} gives a tight upper bound while the alternative one gives a loose bound, so we keep both.
...
heuristics.  
% According to Theorem~\ref{thm:gaussiannoise},
...
Evaluation result(\textbf{0.028}) shows that we choose the best.
