...
\begin{enumerate}
  ...
  % semantics-based dependency graph (in Definition~\ref{def:trace_graph})
  % Semantics-based Dependency Graph (in Definition~\ref{def:trace_graph}). 
...
% % Section~\ref{sec:alg_edgegen}
% \item  Finally, with all the ingredients ready, we construct the final approximated estimated dependency graph in Section~\ref{sec:alg_graphgen}
\end{enumerate}
...
\item 
\jl{
The difference constraints $DC(\mathcal{VAR}  \cup \constdom)$ is the set of all the inequality of
...
%
\item The top constraint, $\top$ denotes true. It is preserved for $\eskip$ command, or commands that don't involve any counter variable.
%
\end{itemize}
\end{itemize}
...
This estimated graph estimated graph has a similar topology structure as 
% the one
% of 
...
%
% We use $\walks(\progG(c))$ represents the walks over the estimated dependency graph for $c$.
...
\begin{defn}[Query Length of the Finite Walk on estimated dependency graph ($\qlen$)]
  ...
  % a program $c$'s semantics-based dependency graph $\traceG(c)(\trace)$
...
% We also show that the estimated adaptivity from our $\pathsearch$ is sound with respect to the estimated adaptivity as in 
...
% The following algorithm finds the walk with the longest query length on a program $c$'s semantics-based dependency graph 
% $\progG(c)$
% %  = (\vertxs, \edges, \weights, \qflag)$, 
...
% Following is the challenge of computing the adaptivity on a estimated dependency graph.
% \\
% In order to 
% % search for the finite walk having the longest query length, which isn't a simple longest weighted path.
% compute the adaptivity for a program $c$ on its estimated estimated dependency graph $\progG(c)$, we need to 
...
% Specifically, this challenge comes from the weight of each vertex estimated in program's estimated dependency graph,
...
% while, in most of our cases, the programs' estimated dependency graphs are having symbolic weights with infinite domains on vertices.
% \todo{for example:}
...
% % \end{equation*}
% \vspace{-0.4cm}
%  \caption{(a) Simple While Loop Example, (b) Semantics-based Dependency Graph, (c) The Static estimated Dependency graph.}
...
%
$\pathsearch(c, \progG(c))$ algorithm in Algorithm~\ref{alg:adapt} arranges the estimated dependency graph $\progG(c)$ into SCCs ($\kw{SCC_1}, \cdots, \kw{SCC_n}$) and obtains the adaptivity local bound of each SCC from $\kw{\pathsearch_{SCC}(c, SCC_i)}$ 
algorithm in Algorithm~\ref{alg:adaptscc}.
