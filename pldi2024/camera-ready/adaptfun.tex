...
\end{center} 
  ...
  \highlight{The complete proof is in supplementary material Appendix~C. Below we show a proof sketch. This lemma is proved by Theorem~\ref{thm:flowsto_soundness}, which shows that for arbitrary two $x^i, y^j \in \lvar_c$, there exists a static ``$\flowsto$ chain'' from $x^i$ to $y^j$ when $\vardep(x^i, y^j, c)$.
\begin{thm}[$\vardep$ implies $\flowsto$]
  \label{thm:flowsto_soundness}
  Given a program ${c}$, for all  $ x^i, y^j \in \lvar_{{c}}$, if $\vardep(x^i, y^j, {c})$,
  then
  ...
  \end{thm}
    ...
    $\flowsto(x^i, y^j, c)$ relation, it is sufficient to show two cases:
...
\\
1. Lemma~\ref{lem:depevents_exist}: existence of a middle event. This is proved by showing a contradiction, with detail in supplementary material Appendix~C.2.%
\\
2. Theorem~\ref{thm:flowsto_event_soundness}: the middle event with a sub-trace implies a ``sub $\flowsto$-chain''. 
