...
To better understand this, we show in Fig.~\ref{fig:generalization_errors} three experiments showcasing these situations: 
...
% More precisely,  
Fig.~\ref{fig:generalization_errors}(a) shows the results of a specific analysis\footnote{We will use formally a program implementing this analysis (Fig.~\ref{fig:overview-example}) as a running example in the rest of the paper.} 
with two rounds of adaptivity.{It can be seen as a classifier for a population where individuals samples consist of 400 attributes and 1 label. Using a dataset representing a sample from this population, this analysis first runs 400 non-adaptive queries on the first 400 attributes of the dataset,
  computing correlations between each attribute and the label. Then, it runs the last query depending on all these correlations. The adaptivity of this analysis is $2$ because only the last query relies on the previous queries' results. 
...
total query number varies along the x-axis. This figure shows how the total number of queries also affect the generalization error.
Fig.~\ref{fig:generalization_errors}(c) highlights the fact that different mechanisms, for the same analysis, produce results with different generalization errors, and
using a larger data set changes the magnitude of the error. } 
In Fig.~\ref{fig:generalization_errors}(b), we show the results of a specific analysis with four hundred rounds of adaptivity.
At each step, this analysis runs an adaptive query based on the results of the previous ones. Without any mechanism, the generalization error is pretty large, and it can be lowered by using Gaussian noise. 
{we show  three different mechanisms: the Gaussian mechanism which adds to the result of each query Gaussian noise; the Data Splitting mechanism
...
This scenario motivates us to explore the design of program analysis techniques to estimate the number of \emph{rounds of adaptivity} that a program implementing a data analysis can perform. These techniques could be used to help a data analyst in the choice of the mechanism to use,
and they
