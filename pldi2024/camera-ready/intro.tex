...
\review{Making Figure 2 more legible, and including a more detailed explanation of the different adaptivity mechanisms.}
 \highlight{This analysis can be seen as a classifier over a population of 400 attributes and 1 label. Using a dataset sampled from this population, this analysis first runs 400 non-adaptive queries on the first 400 attributes of this dataset,
  computing correlations between each attribute and the label. Then, it runs the last query depending on all these correlations. The adaptivity of this analysis is $2$ because only the last query relies on the results of its previous queries' results. 
...
\highlight{We use in total three different mechanisms here: Gaussian mechanism which adds the result of each query Gaussian noise; Data Splitting mechanism
that splits the data into a few parts; Thresholdout mechanism that uses the reusable holdout technique from~\citet{DworkFHPRR15}.  }
{\small
\begin{figure}
\centering
