    ...
    \begin{table}
      ...
      \begin{center}
        \begin{tabular}{| c |c |c| c| }
          ...
          $\mathcal{T}$  &  set of traces & $[]$ &  empty trace \\
          $\ldom$ & set of labels  & $\trace \traceadd \event$  & combine a trace and an event \\
          $\tlabel(\trace) \subseteq \ldom$  &set of the labels occurring in $\trace$  &  $\env (\trace) x$  & fetch the latest value of  $x$ in  $\trace$ \\
          $\mathcal{T}_0(c) \subseteq \mathcal{T}$ &  set of \emph{initial traces} & $\trace \tracecat \trace'$ &  trace concatenation\\
