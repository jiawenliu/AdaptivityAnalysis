...
\begin{table}
  \begin{center}
    \begin{tabular}{| c |c |c| c| }
      $\mathcal{LV}$   & set of labeled variables  & $\mathbb{LV}(c)$  & labeled variables in $c$ \\ 
      $\cdom$  & set of all programs &  $\qvar(c)$ & labeled variables in $c$ with query annotation\\  
      $\mathcal{T}$  &  set of traces & $\trace$ &   a list of \emph{events} \\
      $[]$ &  empty trace  & $\trace \traceadd \event$  & combine a trace and an event \\
      $\trace \tracecat \trace'$ &  trace concatenation  &  $\env (\trace) x$  & fetch the latest value of  $x$ in  $\trace$ \\
      $\tlabel(\trace) \subseteq \ldom$  &set of the labels occurring in $\trace$ & $\vcounter$ & occurrence of a labeled variable in a trace\\
       $\mathcal{T}_0(c) \subseteq \mathcal{T}$ &  set of \emph{initial traces} & $\ldom$ & set of labels
    \end{tabular}
    \end{center}
\end{table}
