...
the number of times this vertex is visited during the program execution. The number relies on 
...
$\kw{lastVal}(\tau, x)$ that returns the latest value of the variable $x$ in any initial trace $\trace$. Also,
it is natural that some vertices are visited only in constant times, such as the vertex $l^{6}$ being executed only once.
