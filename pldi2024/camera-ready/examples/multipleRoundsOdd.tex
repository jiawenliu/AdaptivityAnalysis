...
\highlight{In general, $\THESYSTEM$ is not path-sensitive.} We show this in one example, $\kw{multiRoundsOdd(k)}$, presented in Fig.~\ref{fig:multiRoundsOdd}(a). %% For this program, $\THESYSTEM$ over-approximate the adaptivity.
...
the weight function \highlight{$\kw{lastVal}(\trace_0, \lceil  \frac{k}{2} \rceil  )$   }for the vertex $y^5$ counts the
...
\draw[] (8, 1) circle (0pt) node{{ $x^1: {}^{ 1}_{1}$}};
% Variables Inside the Loop
 \draw[] (0, 7) circle (0pt) node{{ $y^5: {}^{ \lceil \frac{k}{2}\rceil}_{1} $}};
 \draw[] (0, 4) circle (0pt) node{{ $p^6: {}^{ \lfloor \frac{k}{2}\rfloor }_{1} $}};
 \draw[] (0, 1) circle (0pt) node{{ $x^7: {}^{ k }_{1}$}};
 % Counter Variables
 \draw[] (8, 7) circle (0pt) node {{$j^0: {}^{ 1}_{0}$}};
 \draw[] (8, 4) circle (0pt) node {{ $j^3: {}^{  k}_{0}$}};
 %
