...
\begin{defn}
\label{def:diff}
\highlight{Two events $\event_1, \event_2 $ at the same location differ in their values,  if they are either, 
\begin{enumerate}
...
\end{enumerate}
It is denoted as $\diff(\event_1, \event_2)$ as follows:}
{\small
\begin{subequations}
\begin{align}
...
\begin{defn}[Event May-Dependency]
  ...
  , if and only if there exists an assignment event $\event_1'$ with the same location as $\event_1$ but differs in values, either 
  \begin{enumerate}
    \item replacing $\event_1$ with $\event_1'$ in the execution trace will trigger a new execution trace whose newly generated event $\event_2'$ with the same location as $\event_2$ differs in value with $\event_2$, capturing the data dependency.
    \item replacing $\event_1$ with $\event_1'$ in the execution trace will trigger a new execution trace in which a test event after $\event_1$ in the original trace will be flipped, capturing the control dependency.
  \end{enumerate}
  It is denoted as
  $\eventdep(\event_1, \event_2, \highlight{\trace}, c)$, formally defined as follows.}
  \begin{subequations}
  {\small 
  \vspace{-0.1cm}
  \begin{align}
...
\begin{defn}[Semantics-based Dependency Graph]
  ...
  is a function which takes an initial trace $\trace_0$ as input and returns the number of times $x^l$ appears in the execution trace of the program $c$ when starting from $\trace_0$; query annotation on any vertex 
  can be see as a flag indicating if the command associated with this vertex is a query assignment or not.
  }
  The formal definition is defined as follows,
  {\small
  \[
  \begin{array}{lll}
    \text{Vertices} &
