...
This is a simplified version of the \emph{Monitor Augment} from \cite{RogersRSSTW20} with complete implementation in Appendix.
...
$\kw{updcscore}(p,a)$, $\kw{update}(I,ns,cs)$ to simplify the computations of updating $ns$, $cs$ and $I$.%
}
...
% As described in the two round algorithm, the multi-round algorithm has a loop as well.
% compare to two round algorithm
{The interesting part here is that the query request in each iteration is not independent. 
...
% the weights of the vertices $k$ with 
...
%  we have the adaptivity with $\trace_0$ for this program as $w_k(\trace_0)$.
{
...
We know for any initial trace $\trace_0$, $\inferrule{\env(\trace_0) = v}{\config{\trace_0, k} \earrow v}$.
So we have $A(\kw{multipleRounds(k)})(\trace_0) = v$ for any $\trace_0$ by Equation.~\ref{eq:adapt_multipleRounds} above
and $k$ is a tight bound.
}
\end{example}
