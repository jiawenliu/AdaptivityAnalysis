...
%
We look at an advanced adaptive data analysis algorithm - $\kw{multipleRounds}$ algorithm in Figure~\ref{fig:multipleRounds}(a).
...
% For the vertices in Figure~\ref{fig:multipleRounds}(b) that have the weight $k$,
...
\jl{The program-based dependency graph is presented in Figure~\ref{fig:multipleRounds}(b). 
We omitted its execution-based dependency graph $\traceG(\kw{multipleRounds(k)})$ because they have the same graph topology and only differ in weights.
