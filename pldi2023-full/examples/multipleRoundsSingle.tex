\begin{example}[Over-Defined Adaptivtiy Example]
    ...
    As the program in Figure~\ref{fig:multipleRoundSingle}(a),
    ...
    $\kw{multipleRoundSingle(k)}$ is a variant of the multiple rounds strategy 
    % we call it a multiple rounds single iteration algorithm, 
    % named $\kw{multipleRoundSingle(k)}$ 
    with input $k$.
    ...
    As shown in the execution-based dependency graph in Figure~\ref{fig:multipleRoundSingle}(b), 
    ...
    $f_k(\trace_0)$ given the input initial trace $\trace_0$. 
    ...
    is
    $p^7  \to \cdots \to p^7 \to y^4  \to z^1 $ with the vertex $p^7$ visited $f_k(\trace_0)$,
    ...
    \draw[] (-5, 1) circle (0pt) node{{ $z^1: {}^{f_1}_{1}$}};
    \draw[] (-5, 7) circle (0pt) node{{$p^2: {}^{f_1}_{0}$}};
    \draw[] (-5, 4) circle (0pt) node{{ $y^4: {}^{f_1}_{1}$}};
    % Variables Inside the Loop
     \draw[] (0, 6) circle (0pt) node{{ $p^7: {}^{f_k}_{1}$}};
     \draw[] (0, 2) circle (0pt) node{{ $p^{10}: {}^{f_k}_{0}$}};
     % Counter Variables
     \draw[] (5, 6) circle (0pt) node {{$j^0: {}^{f_1}_{0}$}};
     \draw[] (5, 2) circle (0pt) node {{ $j^8: {}^{f_k}_{0}$}};
     %
