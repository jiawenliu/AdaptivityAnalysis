...
\end{tabbing}    
    ...
    \begin{tabbing}
    $ \mathsf{let} \, g =  !\Big(\mathsf{fix} \, (M:\, !_1 (X \to [-1,1] ) \to [-1,1] 
    ) . \lambda j. \lambda k. $ \\
    $\hspace{0.2cm} \mathsf{if}\, (j < k) \, \mathsf{then}$ \\
    $\hspace{0.2cm} \mathsf{let} \, a_j = M !(\lambda X. X(j)\cdot X(k) ) \, \mathsf{in}$\\
    $\hspace{0.4cm} a_j :: (f \, M \,j+1 \, k) $\\
    $\hspace{0.2cm} \mathsf{else} \, [] \Big)$\\
    $\mathsf{in} $\\
    $ \hspace{0.2cm} \mathsf{let}\,l = ! (g \, M\, 0)\, \mathsf{in} $\\
    $\mathsf{let} !Q =  \lambda x. \mathsf{sign} \, (\mathsf{foldl} \, (\lambda a. \lambda r. \lambda i. \big(a\,+ X(i)*lg(\frac{1+r}{1-r})  \big) \, 0 \, l ))  \, \mathsf{in}$\\
    $ M ( Q ) $
\end{tabbing}    
