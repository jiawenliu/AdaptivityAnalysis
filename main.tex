...
\begin{tabbing}
    $\mathsf{let}\, g = \mathsf{fix} \, f(m). \lambda j. \lambda k.$ \\
    $\hspace{0.2cm} \mathsf{if}\, (j < k) \, \mathsf{then}$ \\
    $\hspace{0.2cm} \mathsf{let} \, ! a = m\, (\lambda x. (x \,j)\cdot (x \,k) ) \, \mathsf{in}$\\
    $\hspace{0.4cm} (a, j) :: (f \, m \,j+1 \, k) $\\
    $\hspace{0.2cm} \mathsf{else} \, []  $\\
    $\mathsf{in} $\\
    $ \hspace{0.2cm} \mathsf{let}\, !l = g \, M\, 0\, K \, \mathsf{in} $\\
...
\begin{tabbing}
    $ x: \tint \to  \tint   $ \\
    $ \cdot: \tint * \tint \to \tint $   \\
    $ g: \big( (\tquery \to \tlists{!_1 b * \tint} ) \to \tint \to \tint \to \tlists{!_1 b * \tint}  \big) $\\
    $ q: \tquery $\\
    $ m: \tquery \to !_1 b $\\
    $ M: \tquery \to !_1 b $\\
\end{tabbing}

Type derivation of $g$:\\
Let $A = (\tquery \to \tlists{!_1 b * \tint} ) \to \tint \to \tint \to \tlists{!_1 b * \tint} $. To show $\jtype{1}{}{g}{ A }$. \\
Applying rule FIX, we get:\\
${f: A}, m :\tquery \to \tlists{!_1 b * \tint}  \jtype{1}{}{\lambda j. \lambda k. \mathsf{if} \cdots}{\tint \to \tint \to \tlists{!_1 b * \tint}}$\\
Applying rule ABS twice, we get:\\
${f: A}, m :\tquery \to \tlists{!_1 b * \tint}, j: \tint, k:\tint  \jtype{1}{}{ \mathsf{if} \cdots}{ \tlists{!_1 b * \tint}}$\\
Let $\Gamma = {f: A}, m :\tquery \to \tlists{!_1 b * \tint}, j: \tint, k:\tint$, applying rule IF, we get:\\
$\Gamma \jtype{1}{}{j < k }{\tbool} (\star)$;
$\Gamma \jtype{1}{}{\mathsf{let} \, !a = m \cdots}{\tlists{!_1 b * \tint}} (\diamond)$ and
$\Gamma \jtype{1}{}{\enil}{\tlists{!_1 b * \tint}} (\triangle)$.\\
$(\star)$ can be derived by INL and INR rule, $(\triangle)$ can be derived by NIL rule, the only interesting case is $(\diamond)$.
Applying LET rule, we get:\\
$\Gamma \jtype{1}{}{m\, (\lambda x. (x \,j)\cdot (x \,k) )}{ !_1 b} (\diamond\diamond)$ and $\Gamma, a: [b]_1 \jtype{}{}{(a, j) :: (f \, m \,j+1 \, k) }{\tlists{!_1 b * int}}(\diamond\diamond\diamond)$.\\
Applying APP rule on $(\diamond\diamond)$, we get:\\
$\Gamma, \jtype{1}{}{m}{\tquery \to !_1 b}$ and $\Gamma \jtype{0}{}{\lambda x. (x \,j)\cdot (x \, k)}{\tquery}$. This can be derived by type of $q$ and $m$.\\
$(\diamond\diamond\diamond)$ can be derived by LIST rule.\\

Type derivation of two-rounds algorithm TR:\\
To show: $\jtype{2}{}{TR}{!_2 b}$.
