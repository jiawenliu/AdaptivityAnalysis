...
\begin{figure}
    ...
    \small{
        ...
        %
        \begin{array}{l}
            \kw{loopOdd}(k) \triangleq \\
            \clabel{ \assign{j}{k}}^{0} ; \\
                \ewhile ~ \clabel{j > 0}^{1} ~ \edo ~ \\
                \Big\{
                    \eif \clabel{j \% 2 == 0}^{2} \\
                 \quad \quad \ethen \left\{\clabel{\assign{j}{j - 1}}^{3}\right\} \\
                 \quad \quad \eelse \left\{\clabel{\assign{j}{j - 3}}^{4}\right\}\\
                 \Big\}                         
            \end{array}
        \]
    }
