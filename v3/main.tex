...
\begin{mathpar}
 ...
 \inferrule{
  ...
  {
    \env, (\expr_1, \expr_2) \bigstep{\max(R,S)} (\valr_1, \valr_2)
  }~\textsf{pair}
  %
  \and
  %
  \inferrule{
    \env, \expr \bigstep{R} (\valr_1, \valr_2)
  }{
    \env, \eprojl(\expr) \bigstep{R} \valr_1
  }~\textsf{fst}
  %
  \and
  %
  \inferrule{
    \env, \expr \bigstep{R} (\valr_1, \valr_2), \tr
  }{
    \env, \eprojr(\expr) \bigstep{R} \valr_2, \trprojr(\tr)
  }~\textsf{snd}
  %
  \and
  %
  ...
  \inferrule{
    ...
    \env, \eop(\expr) \bigstep{R } \valr', \trop(\tr)
  }~\textsf{delta2}
\and
%
  \inferrule{
