...
# the example with only sequence, Expected Adaptivity: 4
# Ouput Adaptivity: 4
def test_seq():
...
# the example in if branch and only value dependency, Expected Adaptivity: 2
# Ouput Adaptivity: 2
def test_if_val():
    weights = [AdaptType(1), AdaptType(1), AdaptType(1), AdaptType(1)]
    query = [1, 1, 1, 0]
...
# the example in if branch with both value and control dependency, Expected Adaptivity: 2
# Ouput Adaptivity: 2
def test_if_val_ctl():
...
# the two-round example, Expected Adaptivity: 2
...
# verteices  belong to the same loop),  Expected Adaptivity: 1 + k
