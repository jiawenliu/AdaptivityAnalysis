...
let pp_sort fmt s = match s with
  ...
  | Loc      -> fprintf fmt "%s" ("loc")
  | Arr   -> fprintf fmt "%s" ("Arr")
...
let rec pp_iterm fmt it =
 ...
 let  pp_predicates ppf p =
    List.iter (fun (v, a ) -> fprintf ppf "%a -> @ %a, @ " pp_vinfo v pp_iterm a ) p
...
let rec pp_expr ppf t =
 ...
 (* If-then-else *)
  ...
  | CExpr(_, tm) -> fprintf ppf "celim (%a )" pp_expr tm
  | CLet(_, bi_x, tm1, tm2) ->
    fprintf ppf "@[<v>@[<hov> clet %a as %a @] in@ %a@]" pp_expr tm1 pp_vinfo bi_x  pp_expr tm2
  | Contra _ -> fprintf ppf "contra"
  (*monadic expressions*)
  | Alloc (_, tm1, tm2) -> fprintf ppf "@[Alloc %a @ %a @]" pp_expr tm1 pp_expr tm2
  | Read (_, tm1, tm2) -> fprintf ppf "@[Read %a @ %a @]" pp_expr tm1 pp_expr tm2
  ...
  | SWITCH (_, e,bty,k,c) -> fprintf ppf "@[SWITCH : %a with :%a , k is %a, c is %a @]@\n"
      pp_expr e pp_btype bty pp_iterm k pp_cs c 
