...
\[\boxed{ \config{m, c, D, t} \xrightarrow{} \config{m', c', D, t'} \; }
\]
\[
...
{
\config{m, [\assign{x}{q}]^l, D, t, w} \xrightarrow{} \config{m[ v/ x], \eskip, D, t \mathrel{++} [(q,v)^{(l,w )}],w }
}
~\textbf{query}
%
\\
...
{
\config{m, [\assign{x}{ \expr}]^{l}, D, t,w} \xrightarrow{} \config{m, [\assign{x}{ \expr'}]^{l} , D, t,w}
}
~\textbf{assn1}
%
~~~
...
{
\config{m, [\assign x v]^{l}, D, t,w} \xrightarrow{} \config{m[v/x], [\eskip]^{l}, D,t,w}
}
~\textbf{assn2}
%
\and
...
\inferrule
{
\config{m, c_1, D, t,w} \xrightarrow{} \config{m', c_1', D, t',w}
}
{
\config{m, c_1; c_2, D, t,w} \xrightarrow{} \config{m', c_1'; c_2, D, t',w}
}
~\textbf{seq1}
%
\and
...
{
\config{m, [\eskip]^{l} ; c_2, D , t,w} \xrightarrow{} \config{m, c_2, D, t,w}
}
~\textbf{seq2}
%
\and
...
{
\config{m, \eif([\bexpr]^l, c_1, c_2), D, t,w} 
\xrightarrow{} \config{m,  \eif([\bexpr']^l, c_1, c_2), D, t,w}
}
~\textbf{if}
%
\and
...
{
\config{m, \eif([\etrue]^l, c_1, c_2), D,t,w} 
\xrightarrow{} \config{m, c_1, D, t,w}
}
~\textbf{if-t}
%
~~~~~~~~~~
...
{
\config{m, \eif([\efalse]^l, c_1, c_2), D, t,w} 
\xrightarrow{} \config{m, c_2, D, t,w}
}
~\textbf{if-f}
%
\and
...
{
\config{m, \ewhile([\bexpr]^l, c), D, t,w} 
\xrightarrow{} \config{m,  \eunfold{[\bexpr^{l}] }{\ewhile([\bexpr]^l,   c)} , D, t,w}
}
~\textbf{while} }
%
\and
...
{
\config{m, [\eswitch(\expr, x, (v_i \to q_i))]^{l}, D, t,w} 
\xrightarrow{} \config{m, [ \eswitch(\expr',x, (v_i \to q_i))]^{l}, D, t, w }
}
~\textbf{switch}
}
%
...
{
\config{m, [ \eswitch(v_k,x, (v_i \to q_i))]^{l}, D, t,w} 
\xrightarrow{} \config{m,  [\assign x q_k]^{l}, D, t, w }
}
~\textbf{switch-v}
}
%
...
{
\config{m,  \eloop ~ [\expr_N]^{l} ~ (f) ~ \edo ~ c , D, t, w }
\xrightarrow{} \config{m, [ \eloop ~ [\expr_N]^{l} ~ (f) ~ \edo ~ c]^{l} , D, t, w }
}
~\textbf{loop}
}
%
...
{
\config{m, \eloop ~ [\valr_N]^{l} ~ (f) ~ \edo ~ c , D, t, w }
\xrightarrow{} \config{m, c;  \eloop ~ [(\valr_N-1)]^{l} ~ (f) ~ \edo ~ c , D, t, (w + l) }
}
~\textbf{loop-t}
}
%
...
%
%
\subsection{Independence between two queries in Low level language}
%
...
%
%
\subsection{Dependency between multiple queries }
%
...
%
\begin{example}
\textbf{Dependency graphs for programs containing 3 queries}
\\
Let $q_1$ = $\lambda D. D_i * D_j$, \\ 
Let $q_2 = \lambda D. D_i * D_j + x_1  $.\\
Let $q_3 = \lambda D. D_i * D_j + x_1 - x_2 $, 
$q_3 = \lambda D. D_i * D_j + x_2 $, 
and $q_3 = \lambda D. D_i * D_j + x_1$ .\\
in program $P_1$, $P_2$ and $P_3$ respectively as following:
\[
\begin{array}{c}
P_1 \triangleq\\
      x_1 = \delta ( \lambda D. D_i * D_j) ; \\
   x_2 = \delta(\lambda D. D_i * D_j + x_1  ) ; \\
      \delta(\lambda D. D_i * D_j + x_1 + x_2)
\end{array}
 ~~~
\begin{array}{c}
P_2 \triangleq\\
...
P_3 \triangleq\\
      x_1 = \delta ( \lambda D. D_i * D_j) ; \\
   x_2 = \delta(\lambda D. D_i * D_j + x_1  ) ; \\
      \delta(\lambda D. D_i * D_j  + x_1)
\end{array}
\]
%
\begin{center}
\begin{tikzpicture}
\draw[very thick,->] (8, 0)node[anchor=north]{$q_3$} -- (6, 2) node[anchor=south]{$q_2$};
\draw[very thick,->] (8, 0)  -- (10, 2) node[anchor=south]{$q_1$};
...
%
\begin{tikzpicture}
\draw[very thick,->] (18, 0)node[anchor=north]{$q_3$} -- (16, 2) node[anchor=south]{$q_2$};
\draw[very thick,->] (16.2, 2) -- (19.8, 2)node[anchor=south]{$q_1$};
\draw[rounded corners=8mm, very thick, red, dashed, ->] (18, 0.2) -- (16.4, 1.8) -- (19.6, 1.8);
\end{tikzpicture}
%
\begin{tikzpicture}
\draw[very thick,->] (26, 2) node[anchor=south]{$q_2$} -- (29.9, 2);
\draw[very thick,->] (28, 0)node[anchor=north]{$q_3$}  -- (30, 2) node[anchor=south]{$q_1$};
...
%
Let $q_1 \triangleq \lambda D. D_1 * D_j$, 
$q_2  \triangleq \lambda D.
\eif (x > 1) \ethen (D_3 * D_4); \eelse  (D_3 * D_2) $. 
in program $P_2$ as follows:\\
%
\[
\begin{array}{l}
P_1 \triangleq\\
      x = \delta ( \lambda D.  D_1 * D_j) ; \\
   \eif( x > 1)\\
   \ethen \delta(\lambda D. D_3 * D_4  ) ; \\
     \eelse \delta(\lambda D. D_3 * D_2)
\end{array}
 ~~~
\begin{array}{l}
P_2 \triangleq\\
...
%
\begin{tikzpicture}
\filldraw[black] (10, 2) circle (2pt) node[anchor=south]{$q_2$};
\filldraw[black] (12, 2) circle (2pt) node[anchor=south]{$q_1$};
\filldraw[black] (12, 0) circle (2pt) node[anchor=north]{$q_3$} ;
\end{tikzpicture}
%
\hspace{20pt}
\begin{tikzpicture}
...
\draw[very thick,->] (27, 0)node[anchor=north]{$q_2$}  -- (30, 2) node[anchor=south]{$q_1$};
...
\subsection{Rewrite from High Level Program}
%
...
% \inferrule
% {\empty}
% {\vdash_{G = (V, E)} \assign x \expr_{G' = 
% \left(
% \begin{array}{l}
% V\cup \{x\},\\
% E \cup \left\{(x, v) | v \in VAR(e)  \right\}
% \end{array}\right)
% } 
% % : P \implies P \cup \{x : VAR(e) \} 
% }
% ~\textbf{asgn}
% \and
% \inferrule
% {\empty}
% {\vdash_{G= (V, E)} \assign x q_{
% G' = \left(
% \begin{array}{l}
% V\cup \{x^*\},\\
% E \cup \left\{(x, v) | v \in VAR(e)  \right\}
...
% \and
% \inferrule
% {\vdash_{G} {c_1}_{G_1 = (V_1, E_1)} 
% % : P \implies Q_1
% \and 
% \vdash_{G} {c_2}_{G_2 = (V_2, E_2)}  
% % : P \implies Q_2
% }
% {\vdash_{G = (V, E)} {\eif_D(b, c_1, c_2)}_{
% G' = \left(
% \begin{array}{l}
% V\cup V_1 \cup V_2,\\
% E \cup E_1 \cup E_2 \\
% \cup \left\{(x_1, v) | x_1 \in V_1 \setminus V, v \in VAR(b)\right\}\\
% \cup \left\{(x_2, v) | x_2 \in V_2 \setminus V, v \in VAR(b)\right\} 
% \end{array} \right)}
% : P \implies P 
% }
% %
...
% \and
% \inferrule
% { \vdash_G \phi;c_{G'} : P \land \bexpr = \etrue  \implies P  }
% {\vdash_{G} \ewhile([\bexpr]^{l}, \phi, c)_{G' = ? } : P \implies P \land \bexpr = \efalse }~\textbf{while}
% \andr r r r r r r r rd de
...
% \begin{thm}
% [Soundness]
% Given a program $P$ s.t. $\cdot \vdash_{M,V}^{0, R(P)} P: \Phi \implies \Psi$,
% for all database $D$, s.t. 
% $\lrr{ P }{} (\emptyset, \emptyset , \emptyset, \emptyset, D)  \triangleq  (\sigma, \mu , t, w , D)$,
% then
% \[
% A(P) \leq Adapt(M, V)
% \]
% \end{thm}
% \begin{proof}
%  By induction on the judgment $\cdot \vdash_{M,V}^{0, R(P)} P: \Phi \implies \Psi$.
% \\ 
% \begin{itemize}
% \caseL{Case
% $\inferrule
...
% V= L(x)
% }
% { \cdot \vdash^{(c, c+1)}_{M, V} \assign x q : 
% \Phi \implies \Psi
% }$ }
% \end{itemize}
% For arbitrary database $D$, we have the following semantic.
% \[ 
%   \lrr{ [\assign x q]^{l} }{} (\emptyset, \emptyset , \emptyset, \emptyset, D)   \triangleq  ([x \to v ], \emptyset ,  [(q, v)]^{(l, \emptyset)}, \emptyset , D) \qquad : v = q(D)
% \]
% There is only one node in the trace for all the possible database $D$. Follow the definition of $Adapt(M,V)$, we know that the claim holds.\\
% \begin{itemize}
% \caseL{ Case
% $\inferrule
% {
%  R(e_b) \vdash^{(c_1, c_2)}_{M_1, V_1} P_1 : 
% \Phi \implies \Psi
% \and 
%  R(e_b) \vdash^{(c_2, c_3)}_{M_2, V_2} P_2 :
% \Phi \implies \Psi
% }
% {
% \cdot \vdash^{(c_1, c_3)}_{M_1 \uplus M_2, V_1 \uplus V_2} 
% \eif ~ e_b \ethen P_1 \eelse P_2 :\Phi \implies \Psi
% } $ }
% \end{itemize}
% The semantics depends on the evaluated value of the conditional.
% \[
% \lrr{ \eif_D ([\bexpr]^{l}, c_1, c_2)  }{} (\emptyset, \emptyset , \emptyset, \emptyset , D)   \triangleq  \left \{  \begin{array}{l} 
%          \lrr{c_1}{} (\emptyset, \emptyset , \emptyset, \emptyset , D) \qquad : \lrr{\bexpr}{}(\jl{\emptyset}) = \etrue \\ 
%          \lrr{c_1}{} (\emptyset, \emptyset , \emptyset, \emptyset ,D) \qquad : \lrr{\bexpr}{}(\jl{\emptyset}) = \efalse \end{array} \right . \\  
% \]
% \end{proof}
%
\begin{thm}
