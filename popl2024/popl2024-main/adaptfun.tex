...
\highlight{walk of maximal query length} on this graph, providing in this way an upper bound on the adaptivity of the
program.
...
\[
 \begin{array}{l}
 \forall c \in \cdom, x^l \in \lvar(c),\trace_0 \in \tdom_0(c), \trace \in \tdom,
...
\highlight{\input{adaptfun-graphgen}}
...
\highlight{longest query length walk} to the task of computing local versions of the adaptivity on the maximal strongly connected components of the graph $\progG(c)$ and then compose them into the program adaptivity. 
The algorithm  uses  
