...
{We use acronyms for these programs to have a better presentation of our evaluation in a single table. We use tR for twoRounds,
mRsgl for mRSingle, 3(4)DeLRGD for 3(4)DegreeLRGD,RQ for repeatedQuery, nDPair for nDimPairwise,
DT for decisionTree, LR for logisticRegression, LROVR for logisticRegressionOVR and 
DTOVR for decisionTreeOVR. Similarly, we shorten the mechanisms: GS for Gaussian, DS for Data Split,
TS for Threshold.  }
...
to stress that these are rough heuristics since we are omitting the
...
Then by instantiating $k = 1$, we have $2 * \sqrt{\log(2)} > \sqrt{2}$ and choose Gaussian mechanism, while Gaussian mechanism performs best.
...
$1000* \sqrt{\log(1000)} \gg \sqrt{1000}$ which means that our tool chooses the Gaussian mechanism. When $k=10$ and $n=10$ instead our tool choses the ThresholdOut mechanism. 
...
our tool chooses the Gaussian mechanism which still reduce the generalization error the most.

We have four data analysis which are the implementation of algorithms from~\cite{Jamieson2015TheAO} within the \emph{Guess and Check}~\cite{RogersRSSTW20} framework.
% is the
...
have $k*m$ adaptivity rounds, by instantiating $k = 1000$ and $m = 1000$, {\THESYSTEM} choose Gaussian mechanism, although it is not optimal for the decision tree example.
