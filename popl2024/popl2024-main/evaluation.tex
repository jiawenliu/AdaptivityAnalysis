...
$  \kw{ lRGD3 }$ and $  \kw{ lRGD4 }$ are the generalizations of logistic regression with gradient decedent
...
the Gaussian mechanism performs best when reducing the generalization error. The reason behind this refers back to the Theorem~\ref{thm:gaussiannoise2}, where when the algorithm has ``large'' adaptivity,
we can achieve better generalization error by equipping the Gaussian mechanism with proper parameters.
