...
\highlight{the walk  that visits each vertex $v$ of the semantics-based dependency graph no more than the value that the weight $w_v$ assign to $\tau$, and visits the maximal number of query nodes. }
...
% longest 
\highlight{walk which respect the weights and which visit the maximal query nodes.}
%  as possible. 
...
The algorithm searching for \highlight{the walk} first finds a path $l^6:{}^1_1 \to a^5: {}^k_1 \to x^3: {}^k_1$, and then constructs a walk based on this path. Every vertex on this walk is visited once, and the number of vertices with query annotation $1$ in this walk is $2$, which is the upper bound we expect.
