We want to show an example where $\THESYSTEM$ comes across an over-approximation due to its path-insensitive nature. 
...
\draw[] (8, 1) circle (0pt) node{{ $x^1: {}^{\lambda \trace_0 \st 1}_{1}$}};
% Variables Inside the Loop
 \draw[] (0, 7) circle (0pt) node{{ $y^5: {}^{f_{k/2}}_{1}$}};
 \draw[] (0, 4) circle (0pt) node{{ $p^6: {}^{\lambda \trace_0 \st \env(\trace_0) \frac{k}{2}}_{1}$}};
 \draw[] (0, 1) circle (0pt) node{{ $x^7: {}^{\lambda \trace_0 \st \env(\trace_0) k }_{1}$}};
 % Counter Variables
 \draw[] (8, 7) circle (0pt) node {{$j^0: {}^{\lambda \trace_0 \st 1}_{0}$}};
 \draw[] (8, 4) circle (0pt) node {{ $j^3: {}^{ \lambda \trace_0 \st \env(\trace_0) k}_{0}$}};
 %
...
\draw[ thick, -latex, densely dotted]  (0.5, 6.5) to  [out=-30,in=30] (0.6, 1.6) ;
 % Control Dependency
 \draw[ -latex] (1.5, 7)  -- (7, 6) ;
 \draw[ -latex] (1.5, 4)  -- (7, 6) ;
 \draw[ -latex] (1.5, 1)  -- (7, 6) ;
% Control Dependency
\draw[ -latex] (1.5, 7)  -- (7, 4) ;
\draw[ -latex] (1.5, 4)  -- (7, 4) ;
\draw[ -latex] (1.5, 1)  -- (7, 4) ;
% Edges produced by the transivity
\draw[ -latex] (1.5, 7)  -- (7, 1) ;
\draw[ -latex] (1.5, 4)  -- (7, 1) ;
