...
\highlight{
  Looking into again the running example in Fig.~\ref{fig:overview-example}(a), given initial trace $[ (k, \lin, 1, \bullet)]$,
...
The $x^3$ and $l^6$ are in the variable may-dependency relation by the Def.~\ref{def:var_dep}.}
...
\highlight{For dependency graph of our running example in Fig.~\ref{fig:overview-example}(b),
...
\highlight{For example in Fig.~\ref{fig:overview-example}(b), vertices $a^0$, $j^1$ and $l^6$ have weight function $\lambda \trace \st 1$ because commands $0, 1$ and $6$
...
\end{defn} 
\highlight{
In Fig.~\ref{fig:overview-example}(b), $\lambda \trace_0 \st (l^6 \to x^3)$ is a walk where each vertex is visited only once. 
With the assumption that $k \geq 1$, $\lambda \trace_0 \st (l^6 \to \underbrace{a^4 \to \ldots}_{\env(\trace) k} \to x^3)$ is a walk where vertex $a^4$ is visited $\env(\trace) k$ times.
However, $\lambda \trace_0 \st (l^6 \to a^4 \to x^3 \to x^3)$ is not a walk because there is not an edge from $x^3 \to x^3$.}
...
\highlight{
  Still with the assumption that $k \geq 1$, both walks $\lambda \trace_0 \st (l^6 \to x^3)$ and  $\lambda \trace_0 \st (l^6 \to \underbrace{a^4 \to \ldots}_{\env(\trace) k} \to x^3)$ in Fig.~\ref{fig:overview-example}(b) have query length $\lambda\trace_0 \st 2$ because only
...
\highlight{
  In Fig.~\ref{fig:overview-example}(b), we identified the dotted edge as the longest finite walk which contains the only 
