...
For the control dependency we check whether the change in $\event_1$ affects the appearance in the computation of $\event_2$ or not. 
...
Again, we define the adaptivity by considering only well-formed semantics-based dependency graphs.
  ...
  In Fig.~\ref{fig:overview-example}(b), the two dotted edges represent a finite walk with maximal query length, since it contains the only two query requests.
