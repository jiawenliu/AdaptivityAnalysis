...
\end{defn} 
\highlight{
In Fig.~\ref{fig:overview-example}(b), with the assumption that $k \geq 1$, $\lambda \trace_0 \st (l^6 \to \underbrace{a^4 \to \ldots}_{\env(\trace) k} \to x^3)$ is a walk where vertex $a^4$ is visited at most $\env(\trace) k$ times.
$\lambda \trace_0 \st (l^6 \to a^4 \to x^3 \to x^3)$ is not a walk because there is not an edge from $x^3 \to x^3$.}
...
\highlight{The walk $\lambda \trace_0 \st (l^6 \to \underbrace{a^4 \to \ldots}_{\env(\trace) k} \to x^3)$ in Fig.~\ref{fig:overview-example}(b) has query length $2$ because only
