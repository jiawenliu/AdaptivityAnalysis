...
\wq 
{
  \inferrule
  {
     \env, {\expr_i}^{i \in 1\dots n} 
     \bigstep 
     {\valr_i}^{i \in 1\dots n}, {\tr_i}^{i \in 1\dots n} \\
     \env, {\expr_i'}^{i \in 1\dots n} \bigstep {\valr_i'}^{i \in 1\dots n}, {\tr_i'}^{ i \in 1\dots n}
  }
  {
      \env, \evec 
      (\expr_i \to {\expr_i'}^{i \in 1\dots n}) \bigstep \evec 
      (\valr_i \to {\valr_i'}^{i \in 1\dots n}), \evec 
      ({\tr_i'}^{i \in 1\dots n}) 
  }
}
%
...
\wq{
  \inferrule
    {
      \env, \expr \bigstep \evec 
      (\valr_i \to {\valr_i'}^{i \in 1\dots n}), \tr \\
      \env, \expr' \bigstep \valr', \tr' \\ 
      \valr' = \valr_i 
    }
    {
      \env, \expr \{ \expr' \} \bigstep \valr_i',  \tr  
    }
  }
\end{mathpar}
...
\boxed{  \elet e_i = \mathsf{gererated\_ by\_hidden\_dataset} } \ein \\
  ...
  \hspace{.8cm}  \elet \eapp  a: 0 = 
                 \eop \eapp  
             \Big(\evec 
             \big({\expr_i \to (\eget \eapp \expr_i \eapp j) * (\eget \eapp \expr_i \eapp  k)}^{i \in 1\dots 2^k} 
             \big) 
             \Big) 
                  \eapp \ein \\
  ...
  \hspace{.8cm}  (\mathsf{foldl} \eapp  (\lambda acc: \treal. 
                 ...
                 \big(acc\eapp + (\ddep \eapp \expr_i \eapp  (\eprojr \eapp ai)) 
...
\begin{array}{l}
 ...
 \hspace{0.4cm} \elet \eapp q: k - j = 
   \evec 
   (\expr_i \to \bernoulli \eapp p^{i \in 1\dots n}) \ein \\
 \hspace{0.4cm} \elet \eapp qc: k - j = \evec 
 (\expr_i \to \bernoulli \eapp p^{i \in 1\dots n}) \ein \\
 \hspace{0.4cm} \elet \eapp qj : k - j = \evec 
 ( e_i \to if( (\mathsf{in}\eapp e_i \eapp d) \, , \, q\{e_i\} \,, \mathsf{else}\eapp 0)^{i \in 1\dots n} )\\
...
%
 ...
 \Delta; \Gamma  \tvdash{\nnatA_i'} \expr_i': \type_i' 
     ...
     \Delta;  \Gamma \tvdash{\nnatA'}   \evec(\expr_i \to
     {\expr_i'}^{\, i \in 1,2\dots n})  : \mathsf{VC}(\type_i \to
     {\type_i'}^{\, i \in 1,2\dots n} )
    }
	 ~\textbf{vector}         
