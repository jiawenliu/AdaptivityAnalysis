...
\begin{figure}
  ...
  \end{mathpar}
  \caption{typing rules - quantification}
...
\[
\begin{array}{ll} 
  \mathsf{multiRound} :& \tunit \to \forall  J,K,N,C.
                          \tlist{\treal} \to \tlist{\treal} \to \tlist{\tint} \\
                        & \to \tint[J] \to \tint[K] \to \tint[N] \to \tint[C] \to \tarr{\tlist{\tint}}{\tlist{\tint}}{0}{\dmap}{k - j} \\
  \mathsf{updtSC} : & \tunit \to \forall  J,N. \tlist{\treal} \to \treal \to \treal \to \tbox{ \tint \to \tint } \\
                    & \to \tlist{\tint} \to \tint[J] \to \tint[N] \to \tlist{\tint}\\
  \mathsf{updtSCC} : & \tunit \to  \forall  J,C. \tlist{\treal} \to \treal \to \treal \to \tbox{ \tint \to \tint } \\
                    & \to \tint[J] \to \tint[C] \to \tlist{\tint}\\
  \mathsf{updtI} : & \tunit \to \forall  J,N. \treal \to \tlist{\treal} \to \tint[J] \to \tint[N] \to \tlist{\tint}\\
...
\inferrule*[right = FIX]
{
  \inferrule*[right = FIX]
  {
    \inferrule*[right = IF]
    {
      ...
      {
        \Gamma; \dmap_1 \tvdash{0} D : \tlist{\tint}
      }
    }
    {
  ...
  {
  \Gamma_0 , \mathsf{multiRound} : \tunit \to \cdots; [\mathsf{multiRound}: \infty, () : \bot]
  \tvdash{0?} \Lambda. \Lambda. \cdots : \forall. \forall. \cdots  
  }
}
{
  \Gamma_0; [] \tvdash{(k - j) (0?)} \efix \mathsf{multiRound ()}. \cdots : \tunit \to \cdots
}
    ...
    \inferrule
{
  ...
  {
    \Gamma_8; \dmap_8 
    \tvdash{0} \mathsf{multiRound} \eapp () \eapp \Lambda. \Lambda. \cdot : \tarr{\cdot}{\cdot}{0}{\dmap}{k - (j + 1)}
  }
  \and
  \inferrule
  {
  ...
  {
    \Gamma_8; [D':0] \tvdash{0} D' : \tlist{\tint}
  }
}
{
