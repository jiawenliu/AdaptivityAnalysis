...
\begin{figure}
  \begin{mathpar}
    \begin{array}{lll}
      ...
      \\
       \\
      \lr{\etrue}_{\theta} &=& t\in \mathbb{B}\\
      \lr{\efalse}_{\theta}&=& f\in \mathbb{B}\\
      \lr{(e_1,e_2)}_{\theta}&=& (\lr{e_1}_{\theta}, \lr{e_2}_{\theta})\\
      \lr{x}_{\theta} &=& \theta(x)\\
      \lr{\lambda x:\tau.e}_{\theta} &=&  \hat{\lambda} v\in\lr{\tau}. \lr{e}_{\theta[x=v]}\\
      \lr{e_1e_2}_{\theta} &=&  \lr{e_1}_{\theta}\lr{e_2}_{\theta}\\
      \lr{\return(e)}_{\theta} &=& \hat{\delta}\lr{e}_{\theta}\\
      \lr{\elet x=e_1 \ein e_2 }_{\theta} &=& {\tt mlet}\ v=\lr{e_1}_{\theta}
                                             \ {\tt in}\ \lr{e_2}_{\theta[x=v]} \\
    \end{array}
  \end{mathpar}
...
\begin{figure}
  \begin{mathpar}
    \begin{array}{lll}
      ...
      \\
       \\
      \lrt{\etrue}_{\theta} &=& (t,\etrue) \\
      \lrt{\efalse}_{\theta}&=& (f,\efalse)\\
      \lrt{(e_1,e_2)}_{\theta}&=& let (v_1,T_1)=\lrt{e_1}_{\theta} in 
                                               let (v_2,T_2)=\lrt{e_2}_{\theta}) in 
                                               ((v_1,v_2),(T_1,T_2))\\
      \lrt{x}_{\theta} &=& (\theta(x),x)\\
      \lrt{\lambda x:\tau.e}_{\theta} &=&  (\hat{\lambda} v\in\lr{\tau}. \lr{e}_\theta[x=v]\\
\\
\\
      \lr{\return(e)}_{\theta} &=& \hat{\delta}\lr{e}_{\theta}\\
      \lr{\elet x=e_1 \ein e_2 }_{\theta} &=& {\tt mlet}\ v=\lr{e_1}_{\theta}
