...
{\env, (\expr_1, \expr_2) \bigstep (\valr_1, \valr_2), (\tr_1, \tr_2)}
\]
Pick $\valr = (\valr_1, \valr_2), \tr = (\tr_1, \tr_2), j = |(\tr_1, \tr_2)| = j_1 + j_2$, s.t. $\env, (\expr_1, \expr_2) \bigstep \valr, \tr \land \size{\tr} = j \land j \leq k$.\\
%
STS:\\
...
$(\env, \efix f(x). \expr \bigstep \valr, \tr) \land (\size{\tr} = j) \land (j \leq k)$
...
$|\efix f(x). \expr|= j \land j < k ~(2).$\\
%
Suppose $(1), (2)$, STS: $ (\adap(\efix f(x). \expr) = 0 \leq \nnatA') \land (\forall x. \ddep{x}(\efix f(x). \expr) = \bot \leq \dmap'(x)) \land ((k -j), (\efix f(x). \expr, \env)) \in \lrv{(\tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA})}$.\\
%
We prove on general theorem:
set $k - j = k'$, $\forall m \leq k', (m, (\efix f(x). \expr, \env)) \in \lrv{(\tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA})}$.\\
%
Induction on $m$:
...
%
	...
	%
	Because:\\
	(1). $ (k, \env) \in \lrv{\Gamma}$, applying Lemma 1 on assumption, we get: $(j, \env) \in \lrv{\Gamma}$.\\
	%
...
\]
%
Assume $(k, \env) \in \lrv{\Gamma} ~ (\square)$, TS: $(k, (\env, \expr_1 \eapp \expr_2)) \in \lre{\nnatA'}{\dmap'}{\type_2}$.\\
%
By inversion, pick any $\valr, \tr, j$ s.t. $((\env, \expr_1 \eapp \expr_2) \bigstep (\valr, \tr)) \land (\size{\tr} = j) \land (j \leq k) $.\\
%
STS: $(\adap(\tr) \leq \nnatA') \land (\forall x. \ddep{x}(\tr) \leq \dmap'(x)) \land (((k - j), \valr) \in \lrv{\type_2})$.\\
%
By induction hypothesis on $\star$ and $\square$, we get: $(k, (\env, \expr_1)) \in \lre{\dmap_1}{\nnatA_1}{\tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA}} ~(1) $.\\
%
Inversion on $(1)$, we get:\\
%
Pick any $\valr_1, \tr_1, j_1$, s.t. $((\env, \expr_1) \bigstep (\valr_1, \tr_1)) ~(a) \land (\size{\tr_1} = j_1) \land (j_1 < k)$, \\
%
we know: $(\adap(\tr_1) \leq \nnatA_1) \land (\forall x. \ddep{x}(\tr_1) \leq \dmap_1(x)) \land ((k - j_1), \valr_1) \in \lrv{\tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA}} ~ (2)$\\
%
By inversion on $(2)$, we have: $\valr_1$ is a function.\\
%
Let $\valr_1 = (\efix f(x). \expr, \env') ~(b)$, we have $\forall j' < (k - j_1). \forall (j', \valr') \in \lrv{\type_1}. (j', (\env'[x \mapsto \valr', f \mapsto (\efix f(x).\expr, \env')], \expr)) \in \lre{\dmap[x : \nnatbiA, f: \infty]}{\nnatA}{\type_2} ~ (3)$.\\ 
%
By induction hypothesis on $\diamond$ and $\square$, we get:
%
$(k, (\env, \expr_2)) \in \lre{\dmap_2}{\nnatA_2}{\type_1} ~(4)$.\\
%
Inversion on $(4)$, we get:\\
%
Pick any $\valr_2, \tr_2, j_2$, s.t. $((\env, \expr_2) \bigstep (\valr_2, \tr_2))~(c) \land (\size{\tr_2} = j_2) \land (j_2 \leq k)$,\\
%
we know: $(\adap(\tr_2) \leq \nnatA_2) \land (\forall x. \ddep{x}(\tr_2) \leq \dmap_2(x)) \land ((k - j_2), \valr_2) \in \lrv{\type_1} ~ (5)$\\
%
Apply Lemma 1 on $(5)$, we have $((k - j_2 - j_1), \valr_2) \in \lrv{\type_1}$\\
%
Pick $j' = k - j_1 - j_2, \valr' = \valr_2$, we have: $(k - j_1 - j_2, (\env'[x \mapsto \valr_2, f \mapsto \valr_1], \expr)) \in \lre{\dmap[x : \nnatbiA, f: \infty]}{\nnatA}{\type_2} ~ (6)$ from $(3)$.\\
%
Inversion on $(6)$, let $\env'' = \env'[x \mapsto \valr_2, f \mapsto \valr_1]$, pick any $\valr'', \tr'', j''$, s.t. $((\env'', \expr) \bigstep (\valr'', \tr''))~(d) \land (\size{\tr''} = j'') \land (j'' \leq k - j_1 - j_2)$, we have:\\
%
$\adap(\tr'') \leq n \land (\forall x \in \ddep{x}(\tr'')\leq \dmap[x : \nnatbiA, f: \infty](x)) \land (k - j_1 - j_2 - j'', \valr'') \in \lrv{\type_2}$.\\
%
...
%
(3) $(k - j, \valr) = (k - j_1 - j_2 - j'', \valr'') \in \lrv{\type_2}$\\
%
QED.
