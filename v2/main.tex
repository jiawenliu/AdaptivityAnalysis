...
%
& & & \trnil ~|~ \trcons (\tr_1, \tr_2) ~|~ \boxed{\triapp{\tr_1}{\tr_2} } ~|~ \boxed{\eilam \expr}
\end{array}\]
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \adap( \trlet (x, \tr_1,\tr_2) ) & = & \max (\adap(\tr_2),
                                           \adap(\tr_1)+\ddep{x}(\tr_2)  )
                                           \\
    \boxed{\adap(\triapp{\tr_1}{\tr_2})} & = & \adap(\tr_1) + \adap(\tr_2)\\
    %
    \boxed{\adap(\eilam \expr)} & = & 0
      \end{array}
  \end{mathpar}
  %
  ...
  \begin{mathpar}
    \begin{array}{lcl}
      ...
      \boxed{\ddep{x}(\triapp{\tr_1}{\tr_2})}  & = & 
                                                    \max(\ddep{x}(\tr_1), \adap(\tr_1) + \ddep{x}(\tr_2))\\
    %
    \boxed{\ddep{x}(\eilam \expr)} & = & \bot
    \end{array}
  \end{mathpar}
...
%
  ...
  \type_2 ~|~ \tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA} ~|~
                              \tlist{\type} ~|~ \tbox{\type}\\
& & &                  \boxed{ ~|~  \treal ~|~  \tint ~|~   \tint[I]  ~|~      \tforall{\dmap}{\nnatA}{i} \type  ~|~ \texists{i} \type      } 
%
\end{array}
\]
...
\begin{thm}[Fundamental theorem]
  ...
  \in \lrv{\Gamma}$ and $ (\stepiA, \ienv) \in \lrv{\Delta}$, then $(\stepiA, (\env, \ienv \expr)) \in
  \lre{\dmap}{\ienv \nnatA}{\ienv \type}$.
\end{thm}
\end{thm}
%
...
\]
Assume $(\stepiA, \env) \in \lrv{\Gamma, \Gamma'}$, $(\stepiA, \ienv) \in \lrv{\Delta}$, to show: $(\stepiA, (\env, \ienv \expr)) \in \lre{\dmap}{\ienv \nnatA}{\ienv \tbox{\type}}$.\\
%
...
\]
Assume $(\stepiA, \env) \in \lrv{\Gamma } ~ (\diamond)$, $(\stepiA, \ienv) \in \lrv{\Delta}~(\star)$, TS $(\stepiA, (\env, \eilam \ienv \expr)) \in \lre{\dmap}{\ienv \nnatA}{\tforall{\dmap}{\nnatA}{i} \type} $.\\
%
...
%
By $(\star)$, we know $(\stepiA, \ienv[i \to I]) \in \lrv{\Delta, i::S} ~ (\triangle)$.\\
%
...
\]
Assume $(\stepiA, \env) \in \lrv{\Gamma}$, $(\stepiA, \ienv) \in \lrv{\Delta}$, TS: $(\stepiA, (\env, \ienv (\expr []))) \in \lre{\dmap}{\ienv(\nnatA[I/i] + \nnatA_1)}{\ienv(\type[I/i])}$.\\
%
...
\]
Assume $(\stepiA, \env) \in \lrv{\Gamma}$, $(\stepiA, \ienv) \in \lrv{\Delta}$ TS: $(\stepiA, \ienv (\eop(\expr), \env)) \in \lre{\dmap'}{\ienv \nnatA'}{\treal}$.\\
%
...
%
By typing rule PRIMITIVE, we have $\eop(\valr'): \treal$. So, we have $(\stepiA - j' - 1, \eop(\valr')) \in \lrv{\treal}$.\\
%
}
