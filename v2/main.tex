...
\label{prop:fund}
  If $\Gamma; \dmap \tvdash{\nnatA} \expr: \type$ and $(k, \env) \in
  \lrv{\Gamma}$, then $(k, (\env, \expr)) \in \lre{\dmap}{\nnatA}{\type}$.
\end{prop}
\begin{proof} of Proposition \ref{prop:fund}.\\
Assume we know $(k, \env) \in \lrv{\Gamma}$, 
by induction on $\Gamma; \dmap \tvdash{\nnatA} \expr: \type$:\\
\noindent {\bf case}
...
%
We prove on general theorem: set $k - j = k'$, $\forall m \leq k', (m, (\efix f(x). e, \env)) \in \lrv{(\tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA})}$.\\
%
Induction on $m$:
