...
3. $(\stepiA - (j_1 + j_2), (\valr_1, \valr_2)) \in \lrv{\type_1
  \times \type_2}$, which is proved by (10) and applying Lemma~\ref{lem:downward} on $(6)$.\\
...
%
we know: $(\adap(\tr_2) \leq \nnatA)~(6) \land (\forall x. \in \ddep{x}(\tr_2) \leq \dmap(x))~(7) \land ((\stepiA-1 - j_1-j_2, \valr_2) \in \lrv{\type})~(8)$.\\
%
...
%
we know: $(\adap(\tr_3) \leq \nnatA) ~(9) \land (\forall x. \in \ddep{x}(\tr_3) \leq \dmap(x))~(10) \land ((\stepiA -1-j_1- j_3, \valr_3) \in \lrv{\type})~(11)$.\\
%
Apply E-IFT on $(a) (b)$, we get:
\[
  \inferrule{
...
not negative.\\
2. $\forall x \in \ddep{x}(\enil)$, $  \ddep{x}(\enil) = \bot \leq
