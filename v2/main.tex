...
\begin{mathpar}
 ...
 \inferrule{ \env, \expr_1 \bigstep \econst, \tr_1 \\ \env, \expr_2 \bigstep \econst, \tr_2  }{\env, \uniform \eapp \expr_1 \eapp
  ...
  \wq{ \inferrule{
    ...
    \env, \efix f(x:\tau). \expr \bigstep (\efix f(:\tau).\expr, \env),
    (\trfix f(x:\tau).\expr, \env)
  }
}
  %
  \and
  ...
  \inferrule{
    ...
    \wq{ \valr_1 = (\efix f(x:\tau).\expr, \env')} \\\\
...
\begin{figure}
  ...
  \begin{mathpar}
    \begin{array}{lcl}
      ...
      %
      \wq {\adap( (\trfix f(x:\tau).\expr, \env)  ) } & = & 0 \\
      %
      ...
      \adap(\trop(\tr)) & = &\wq { 1 + \adap(\tr) } \\
           & &       \wq{  +  \textsf{MAX}_{\valr \in \tau} \Big(
                              \max \big(\adap(\tr_3 (\valr) ),
                              \ddep{x}(\tr_3(\valr)) \big) \Big) } \\
      &\mathsf{where}& \wq { \valr_1 = (\efix f(x: \tau). \expr, \env ) =
                       \mathsf{extract}(\tr) } \\
 & & \wq{ \conj  \env[f \mapsto
  ...
  \begin{mathpar}
    \begin{array}{lcl}
      ...
      %
     \wq { \ddep{x}(  (\trfix f(y:\tau).\expr,\env)  )  }& = & \bot \\
      %
      ...
      & &  \adap(\tr) + \textsf{MAX}_{\valr \in \tau} \Big(
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule{
      ...
      \Delta; \Gamma; \dmap' \tvdash{\nnatA'} \efix f(x: \tau_1).\expr: (\tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA})
    }
    %
    \and
    %
  ...
  \and
    %
     \inferrule{
      ...
      \Delta  \models \sub{ \tbox{\tau}  }{ \tau }
    }
  \and
    %
     \inferrule{
    }{
      \Delta  \models \sub{ \tbox{\tau}  }{ \tbox{\tbox{\tau}} }
    }
  \and
    %
     \inferrule{
      ...
      \Delta \models \sub{ \tbox{\tau_1}  }{ \tbox{\tau_2} }
    }
  \and
    %
     \inferrule*[right = refl]{
    }{
      \Delta \models \sub{ \tau  }{ \tau }
    }
 \and
    %
     \inferrule*[right=tran]{
      \Delta \models \sub{ \tau_1  }{ \tau_2 } 
        \\
      \Delta \models \sub{ \tau_2  }{ \tau_3 }
    }{
      \Delta \models \sub{ \tau_1  }{ \tau_3 }
    }
\and
%
...
{
	\Delta; \Gamma; \psi_a; \dmap \tvdash{\color{red} \nnatA }
...
{
	\Delta; \Gamma; \psi_a; \dmap \tvdash{\red{ 0} } \infr{c}{ 
            \tbase }{\bot}{.}
}
%
\quad
...
\inferrule
{
	\Delta; \dmap \tvdash{} \type: wf
}
{
	\Delta; \Gamma; \psi_a; \dmap \tvdash{\color{red} 0} \infr{\enil}{ \tlist{\type} }{\bot}{.}
}
%
\and
%
...
{
	\Delta; \Gamma; \psi_a; \dmap \tvdash{\color{red} 0 } \infr{\etrue}{\tbool}{\bot}{.}
}
%
\quad
%
...
{
	\Delta; \Gamma; \psi_a;\dmap \tvdash{ \color{red} 0} \infr{\efalse}{\tbool}{\bot}{.}
}
%
\and
%
\inferrule*[right = alg-FIX]
{
	\Delta; \Gamma, f :
        \tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA}, x : \type ;
      \psi_a;  \dmap[f : \infty, x : \nnatbiA]
	\tvdash{ \nnatA} \chec{\expr}{\type_2}{\restriction}
}
{
	\Delta; \Gamma; \psi_a;\dmap' \tvdash{ \nnatA'} 
	\chec{\efix f(x). \expr}{
          \tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA}
        }{\restriction \land \nnatA' \geq 0 }
}
...
\and
%
\inferrule*[right =  alg-app]
{
	\Delta; \Gamma; \psi_a;\dmap_1 \tvdash{\red{\nnatA_1} } 
	\infr{\expr_1}{ \tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA} }{ \restriction_1 }{\psi}
	\and
       \nnatA_2 \in \fresh(\natb)\\
	\Delta; \Gamma; \psi_a, \psi; \dmap_2 \tvdash{\nnatA_2}
        \chec{\expr_2}{\type_1}{\restriction_2} \and
       \restriction_3 = \nnatA' \geq \nnatA_1 + \max(\nnatA,\nnatA_2 + \nnatbiA)
}
{
	\Delta; \Gamma; \psi_a; \dmap'  \tvdash{\red {\nnatA'} } 
	\infr{\expr_1 \eapp \expr_2}{\type_2}{\restriction_1 \land \restriction_2 
	\land \restriction_3 \land \dmap' \geq \max(\dmap_1, \nnatA_1 + \max(\dmap, \dmap_2
        + \nnatbiA))}{\psi, \nnatA_2}
}
%
\and
%
\inferrule*[right=alg-pair]
{      \nnatA_1, \nnatA_2 \in \fresh(\natb) \and
	\Delta; \Gamma; \psi_a; \dmap_1 \tvdash{\nnatA_1} \chec{\expr_1}{\type_1}{\restriction_1}
	\and
	\Delta; \Gamma; \psi_a; \dmap_2 \tvdash{\nnatA_2} \chec{\expr_2}{\type_2}{\restriction_2}
}
{
	\Delta; \Gamma; \psi_a; \dmap \tvdash{\nnatA} \chec{(\expr_1, \expr_2)}{\type_1 \times \type_2}
	{\exists \nnatA_1. \restriction_1 \land \exists \nnatA_2.\restriction_2 \land \nnatA \geq \max(\nnatA_1, \nnatA_2) 
	\land \dmap = \max(\dmap_1, \dmap_2)}
}
%
\and
%
...
\and
%
\inferrule*[right = alg-case]
{
	\Delta; \Gamma; \psi_a;\dmap \tvdash{\red {\nnatA_1 }} \infr{\expr_1}{\tbool}{\restriction_1}{\psi}
	\and
      \nnatA \in \fresh( \natb )\and
	\Delta; \Gamma; \psi_a, \psi;\dmap \tvdash{\nnatA} \chec{\expr_2}{\type}{\restriction_2}
	\\
	\Delta; \Gamma; \psi_a, \psi;\dmap \tvdash{\nnatA}
        \chec{\expr_3}{\type}{\restriction_3} \and
        \restriction = \exists \psi. \restriction_1 \land \exists \nnatA. \restriction_2 \land \restriction_3
	\land \nnatA' \geq \nnatA_1 + \nnatA
	\land \dmap' \geq \max(\dmap_1, \nnatA_1 + \dmap)
}
{
...
\inferrule
{
       \nnatA \in \fresh(\natb)\\	
       \Delta; \Gamma; \psi_a; \dmap \tvdash{\nnatA} 
	\chec{\expr}{ \tbox{\tarr{\type_1}{\type_2}{0}{\dmap''}{0} } }{\restriction}
}
{
	\Delta; \Gamma; \psi_a;\dmap' \tvdash{\red {\nnatA'} }
	\infr{\eop{\expr}}{\treal}
...
\inferrule
{
	\Delta; \tbox{\Gamma}; \psi_a; \dmap \tvdash{\nnatA} \chec{\expr}{\type}{\restriction}
}
{
	\Delta; \Gamma',\tbox{\Gamma}; \psi_a; \dmap \tvdash{\nnatA} \chec{\mathsf{BOX}\expr}{\tbox{\type}}{\restriction}
}
}
%
\and
...
{       \nnatA_1, \nnatA_2 \in \fresh ( \natb ) \and
	...
	\and
	\Delta; \Gamma; \psi_a;\dmap_2 \tvdash{\nnatA_2} \chec{\expr_2}{ \tlist{\type} }{\restriction_2}
}
{
	\Delta; \Gamma; \psi_a; \dmap \tvdash{\nnatA} \chec{\econs(\expr_1, \expr_2)}{\tlist{\type}}
	{
	\exists \nnatA_1. \restriction_1 \land \exists \nnatA_2. \restriction_2
	\land
	\nnatA = \max(\nnatA_1, \nnatA_2)
	\land
	\dmap = \max(\dmap_1, \dmap_2)
...
\inferrule
{
	...
	\and
       \nnatA_2 \in \fresh(\natb)  \\
	\Delta; \Gamma, x : \type_1; \psi_a,\psi;\dmap_2[x: \nnatbiA] \tvdash{\nnatA_2} 
	\chec{\expr_2}{\type_2}{\restriction_2} 
}
{
	\Delta; \Gamma; \psi_a; \dmap \tvdash{\nnatA} \chec{\elet x = \expr_1 \ein \expr_2 }{\type_2}
	{
	\exists \psi .\restriction_1 \land \exists \nnatA_2.\restriction_2
	\land
	\nnatA = \max(\nnatA_2, \nnatA_1 + \nnatbiA)
	\land
...
}
{
	\Delta; \Gamma; \psi_a; \dmap' \tvdash{\nnatA'}
	\chec{\expr}{\type'}
	{\exists \psi. \restriction \land \restriction'
	\land \nnatA' \geq \nnatA}
}
}
%
...
\inferrule
{
	i::S, \Delta; \Gamma; \psi_a;\dmap \tvdash{\nnatA}
	\chec{\expr}{\type}{\restriction}
}
{
	\Delta; \Gamma; \psi_a;\dmap \tvdash{\nnatA'}
	\chec{\eilam \expr}{ \tforall{\dmap}{\nnatA}{i}
        }{\forall i. \restriction \land \nnatA' = 0}
}
%
\and
%
...
}
{
	...
	{
	\restriction \land \dmap' \geq \max(\dmap, \nnatA + \dmap_1)
	\land \nnatA' \geq \nnatA_1[I/i] + \nnatA}{\psi}
} 
\end{mathpar}
