...
\wq{ (\trfix f(x).e, \env) } ~|~ (\tr_1, \tr_2) ~|~ \trprojl(\tr) ~|\\ 
%
& & & \trprojr(\tr) ~|~ \trtrue ~|~ \trfalse ~|~ \trift(\tr_b, \tr_t)
~|~ \triff(\tr_b, \tr_f) ~|~ \trconst ~|~ \trop(\tr) \\
%
& & & \trnil ~|~ \trcons (\tr_1, \tr_2) ~|~ \triapp{\tr_1}{\tr_2} ~|~ \eilam \expr
\end{array}\]
 ...
 & & \wq{ \conj  \env[f \mapsto
  ...
  \begin{mathpar}
    \begin{array}{lcl}
      ...
      \ddep{x}(\trconst) & = & \bot \\
      %
      \ddep{x}(\trop(\tr)) & = & \wq {1 +  \max(\ddep{x}(\tr), } \\
      & &  \adap(\tr) + \textsf{MAX}_{\valr \in \tau} \Big( \max(\ddep{x}(\tr_3(\valr)),
          \ddep{y}(\tr_3(\valr)) )   \Big ) ) \\  
 &\mathsf{where}& \wq { \valr_1 = (\efix f(x: \tau). \expr, \env ) =
                       \mathsf{extract}(\tr) } \\
 & & \wq{ \conj  \env[f \mapsto
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule
    {
     ...
     \wq{ delete : \forall x \in \dmap''. \dmap''(x) = 0 }\\
      \nnatA' = 1 + \nnatA \\
      \dmap' = 1 + \dmap
    }
    {
...
{
	\Delta; \Gamma; \dmap' \tvdash{\nnatA'} 
	\chec{\efix f(x). \expr}{ \tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA} }{\restriction}
}
%
\and
%
...
\item $ \delta \not\in
 \valr~(b)$.  We know $\delta \not\in \expr$. \wq{add a lemma? If
   $\delta \not \in \expr$ and $\forall \env, \valr, \tr . \env,\expr
   \bigstep (\valr,\tr) $, then $\delta \not \in \valr$. }
\end{enumerate}
