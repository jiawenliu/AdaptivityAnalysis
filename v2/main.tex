...
$(\env, \efix f(x). \expr \bigstep \valr, \tr) \land (|\tr| = j) \land (j \leq k)$
...
%
By inversion, pick $\valr, \tr, j$ s.t. $((\env, \expr_1 \expr_2) \bigstep (\valr, \tr)) \land (|\tr| = j) \land (j \leq k) $.\\
%
STS: $(\adap(\tr) \leq \nnatA') \land (\forall x. \ddep{x}(\tr) \leq \dmap'(x)) \land (((k - j), \valr) \in \lrv{\type_2})$.\\
%
By induction hypothesis on $\star$ and $\diamond$,
