...
%
By inversion, pick any $\valr_1, j_1, \tr_1$ s.t.,
...
we have: \\
$(\adap(\tr_1) \leq \nnatA_1) ~ (c)$ 
$\land \forall x, \ddep{x}(\tr_1) \leq \dmap_1(x) ~ (e)$ 
$\land (\stepiA - j_1, \valr_1) \in \lrv{\type_1} ~ (m) $.\\
%
By induction hypothesis on $(2)$, we have $(\stepiA, \expr_2) \in \lre{\dmap_2}{\nnatA_2}{\type_2}$.\\
%
By inversion, pick $\valr_2, j_2, \tr_2$ s.t., $(b) ~ (\env, \expr_2 \bigstep \valr_2, \tr_2) \land (\size{\tr_2} = j_2) \land j_2 < \stepiA $,\\
%
we have:\\
$\adap(\tr_2) \leq \nnatA_2 ~ (d)$
$\land \forall x, \ddep{x}(\tr_2) \leq \dmap_2(x) ~ (f)$
$\land (\stepiA - j_2, \valr_2) \in \lrv{\type_2} ~ (n)$.\\
%
