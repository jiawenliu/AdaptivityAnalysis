...
\begin{figure}
  ...
  \begin{mathpar}
    \begin{array}{lcl}
      ...
      \size{\trfalse} & = & 0 \\
      %
      \size{\trift(\tr_b, \tr_t)} & = & \size{\tr_b} + \size{\tr_t} \\
      %
      \size{\triff(\tr_b, \tr_f)} & = & \size{\tr_b} + \size{\tr_f} \\
      %
      \size{\trconst} & = & 0 \\
      %
      \size{\trop(\tr)} & = & 1 + \size{\tr}
...
to show: $\stepiA, \env, (\expr_1, \expr_2) \in \lrv{\type_1 \times \type_2}$.\\
%
By inversion, STS: $\forall \, \valr, j, \Gamma.$ $\env, \expr_1, \expr_2 \bigstep \valr, \, \tr \land \size{\tr} = j \land j \leq \stepiA \implies \adap(\tr) < \nnatA' \land \forall x, \ddep{x}(\tr) \leq \dmap(x) \land (\stepiA - j, \valr) \in \lrv{\type_1 \times \type_2}$.\\
%
By induction hypothesis on $(1)$, we have $(\stepiA, \expr_1) \in \lre{\dmap_1}{\nnatA_1}{\type}$.\\
%
By inversion, pick $\valr_1, j_1, \tr_1$ s.t., $(a) ~ \env, \expr_1 \bigstep \valr_1, \tr_1 \land \size{\tr_1} = j_1 \land j_1 < \stepiA $, we have: $(c)~\adap(\tr_1) \leq \nnatA_1 \land (e)~\forall x, \ddep{x}(\tr_1) \leq \dmap_1(x) \land (m)~(\stepiA - j_1, \valr_1) \in \lrv{\type_1} $.\\
%
By induction hypothesis on $(2)$, we have $(\stepiA, \expr_1) \in \lre{\dmap_2}{\expr_2}{\type_2}$.\\
%
By inversion, pick $\valr_2, j_2, \tr_2$ s.t., $(b) ~ (\env, \expr_2 \bigstep \valr_2, \tr_2) \land (\size{\tr_2} = j_2) \land j_2 < \stepiA $, we have: $(d)~\adap(\tr_2) \leq \nnatA_2 \land (f)~\forall x, \ddep{x}(\tr_2) \leq \dmap_2(x) \land (n)~(\stepiA - j_2, \valr_2) \in \lrv{\type_2} $.\\
...
%
3. $\stepiA - (j_1 + j_2), (\valr_1, \valr_2) \in \lrv{\type_1 \times \type_2}$, which is proved by apply Lemma \ref{lem:ddep-leq-adap} on $(m), (n)$.
...
%
By E-FIX, we know: {\color{red}$\valr = (\efix f(x). \expr, \env)$:}\\
...
%
	...
	%
	By Sub induction hypothesis, we have:
	$(m', (\efix f(x). \expr, \env)) \in \lrv{(\tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA})} ~ (a)$.\\
	%
	Pick $\env' = \env[x \mapsto \valr_m, f \mapsto (\efix f(x).\expr, \env)]$,\\
	%
	we know: $(j, \env') \in \lrv{\Gamma, f:(\tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA}),  x: \type_1} ~ (\diamond)$.\\
	%
	Because:\\
...
%
we know: $(\adap(\tr_1) \leq \nnatA_1) \land (\forall x. \ddep{x}(\tr_1) \leq \dmap_1(x)) \land ((\stepiA - j_1), \valr_1) \in \lrv{\tarr{\type_1}{\type_2}{\nnatbiA}{\dmap}{\nnatA}} ~ (2)$\\
%
By inversion on $(2)$, we have: $\valr_1$ is a function.\\
%
Let $\valr_1 = (\efix f(x). \expr, \env') ~(b)$, we have $\forall j' < (\stepiA - j_1). \forall (j', \valr') \in \lrv{\type_1}. (j', (\env'[x \mapsto \valr', f \mapsto (\efix f(x).\expr, \env')], \expr)) \in \lre{\dmap[x : \nnatbiA, f: \infty]}{\nnatA}{\type_2} ~ (3)$.\\ 
%
By induction hypothesis on $\diamond$ and $\square$, we get:
%
$(\stepiA, (\env, \expr_2)) \in \lre{\dmap_2}{\nnatA_2}{\type_1} ~(4)$.\\
%
Inversion on $(4)$, we get:\\
%
Pick any $\valr_2, \tr_2, j_2$, s.t. $((\env, \expr_2) \bigstep (\valr_2, \tr_2))~(c) \land (\size{\tr_2} = j_2) \land (j_2 \leq \stepiA)$,\\
%
we know: $(\adap(\tr_2) \leq \nnatA_2) \land (\forall x. \ddep{x}(\tr_2) \leq \dmap_2(x)) \land ((\stepiA - j_2), \valr_2) \in \lrv{\type_1} ~ (5)$\\
%
Apply Lemma 1 on $(5)$, we have $((\stepiA - j_2 - j_1), \valr_2) \in \lrv{\type_1}$\\
%
Pick $j' = \stepiA - j_1 - j_2, \valr' = \valr_2$, we have: $(\stepiA - j_1 - j_2, (\env'[x \mapsto \valr_2, f \mapsto \valr_1], \expr)) \in \lre{\dmap[x : \nnatbiA, f: \infty]}{\nnatA}{\type_2} ~ (6)$ from $(3)$.\\
%
Inversion on $(6)$, let $\env'' = \env'[x \mapsto \valr_2, f \mapsto \valr_1]$, pick any $\valr'', \tr'', j''$, s.t. $((\env'', \expr) \bigstep (\valr'', \tr''))~(d) \land (\size{\tr''} = j'') \land (j'' \leq \stepiA - j_1 - j_2)$, we have:\\
%
$\adap(\tr'') \leq n \land (\forall x \in \ddep{x}(\tr'')\leq \dmap[x : \nnatbiA, f: \infty](x)) \land (\stepiA - j_1 - j_2 - j'', \valr'') \in \lrv{\type_2}$.\\
%
...
\]
Pick $\valr = \valr'', j = j_1 + j_2 + j'', \tr = \trapp{\tr_1}{\tr_2}{f}{x}{\tr''}$, the following 3 is proved:\\
%
1. $\adap(\tr) = \adap(\trapp{\tr_1}{\tr_2}{f}{x}{\tr''}) = \adap(\tr_1) + \max(\adap(\tr''), \adap(\tr_2) + \ddep{x}(\tr'')) \leq \nnatA_1 + \max(\nnatA, \nnatA_2 + \nnatbiA) = \nnatA'$\\
%
2. $\ddep{x}(\tr) = \max(\ddep{x}(\tr_1), \adap(\tr_1) + \max(\ddep{x}(\tr''), \ddep{x}(\tr_2) + \ddep{y}(\tr''))) \leq \max(\dmap_1, \nnatA_1 + \max(\dmap, \dmap_2 + \nnatbiA)) = \dmap'$\\
%
3. $(\stepiA - j, \valr) = (\stepiA - j_1 - j_2 - j'', \valr'') \in \lrv{\type_2}$\\
%
QED.
...
%
By induction hypothesis on $\diamond$, we get: $(\stepiA, (\env, \expr_2)) \in \lre{\dmap}{\nnatA}{\type} ~(2)$\\
%
By induction hypothesis on $\triangle$, we get: $(\stepiA, (\env, \expr_3)) \in \lre{\dmap}{\nnatA}{\type} ~(3)$\\
%
Inversion on $(1)$, pick any $ \valr_1, \tr_1, j_1. (\env, \expr_1 \bigstep \valr_1, \tr_1) ~ (a) \land (\size{\tr_1} = j_1) \land (j_1 \leq \stepiA)$,\\
%
we know: $(\adap(\tr_1) \leq \nnatA_1) \land (\forall x. \in \ddep{x}(\tr_1) \leq \dmap_1(x)) \land ((\stepiA - j_1, \valr_1) \in \lrv{\type})$\\
%
Induction on $\valr_1$, we have two following subcases:

{\bf Subcase1:} $\valr_1 = \etrue$, \\
%
inversion on $(2)$, pick any $\valr_2, \tr_2, j_2. (\env, \expr_2 \bigstep \valr_2, \tr_2) ~ (b) \land (\size{\tr_2} = j_2) \land (j_2 \leq \stepiA)$,\\
%
we know: $(\adap(\tr_2) \leq \nnatA) \land (\forall x. \in \ddep{x}(\tr_2) \leq \dmap(x)) \land ((\stepiA - j_2, \valr_2) \in \lrv{\type})$.\\
%
...
\]
Pick $\valr = \valr_2, \tr = \trift(\tr_1, \tr_2), j = j_2$, the following 3 items are proved:\\
%
1. $\adap(\tr) = \adap(\trift(\tr_1, \tr_2)) = \adap(\tr_1) + \adap(\tr_2)) \leq \nnatA_1 + \nnatA = \nnatA'$.\\
%
2. $\forall x \in \ddep{x}(\tr) = \max(\ddep{x}(\tr_1), \adap(\tr_1) + \ddep{x}(\tr_2)) \leq \max(\dmap_1(x), \nnatA_1(x) + \dmap(x)) = \dmap'(x) $\\
%
3. $(\stepiA - j, \valr) = (\stepiA - j_2, \valr_2) \in \lrv{\type}$

{\bf Subcase2:} $\valr_1 = \efalse$\\
%
inversion on $(3)$, pick any $\valr_3, \tr_3, j_3. (\env, \expr_3 \bigstep \valr_3, \tr_3) ~ (b) \land (\size{\tr_3} = j_3) \land (j_3 \leq \stepiA)$,\\
%
we know: $(\adap(\tr_3) \leq \nnatA) \land (\forall x. \in \ddep{x}(\tr_3) \leq \dmap(x)) \land ((\stepiA - j_3, \valr_3) \in \lrv{\type})$.\\
%
Apply E-IFT on $(a) (b)$, we get:
\[
  \inferrule{
...
\]
Pick $\valr = \valr_3, \tr = \trift(\tr_1, \tr_3), j = j_3$, the following 3 items are proved:\\
%
1. $\adap(\tr) = \adap(\trift(\tr_1, \tr_3)) = \adap(\tr_1) + \adap(\tr_3)) \leq \nnatA_1 + \nnatA = \nnatA'$.\\
%
2. $\forall x \in \ddep{x}(\tr) = \max(\ddep{x}(\tr_1), \adap(\tr_1) + \ddep{x}(\tr_3)) \leq \max(\dmap_1(x), \nnatA_1(x) + \dmap(x)) = \dmap'(x) $\\
%
3. $(\stepiA - j, \valr) = (\stepiA - j_3, \valr_3) \in \lrv{\type}$
...
%
$(\stepiA - j', \valr_1) \in \lrv{\type_1} \land (\stepiA - j', \valr_2) \in \lrv{\type_2}$.\\
%
By E-FST, pick $\valr = \valr_1, j = j', \tr = \trprojl(\tr')$, the following 3 items are proved:\\
%
1. $\adap(\tr) = \adap(\trprojl(\tr')) = \adap(\tr') \leq \nnatA$ by $(a)$\\
%
2. $\forall x \in \ddep{x}(\tr) = \forall x \in \ddep{x}(\tr') \leq \dmap(x)$\\
%
3. $(\stepiA - j, \valr) = (\stepiA - j', \valr_2) \in \lrv{\type_1}$\\
QED.
...
$(\stepiA - j', \valr_1) \in \lrv{\type_1} \land (\stepiA - j', \valr_2) \in \lrv{\type_2}$.\\
%
By E-FST, pick $\valr = \valr_2, j = j', \tr = \trprojl(\tr')$, the following 3 items are proved:\\
%
1. $\adap(\tr) = \adap(\trprojl(\tr')) = \adap(\tr') \leq \nnatA$ by $(a)$\\
%
2. $\forall x \in \ddep{x}(\tr) = \forall x \in \ddep{x}(\tr') \leq \dmap(x)$\\
...
%
Pick $\valr = \eop(\valr')$ for any $\valr'$, then by E-PRIMITIVE, we have $\env, \eop(\expr) \bigstep \valr, \eop(\tr')$.\\
%
Pick $\tr = \eop(\tr'), j = j' + 1$, the following 3 items are proved:\\
%
...
$~~$By definition of $(\stepiA, \env) \in \lrv{\Gamma}$, we have: $(\stepiA, \env(x))\in \lrv{\Gamma(x)}$, and $\Gamma(x) = \type$. This item is proved.\\
QED.
