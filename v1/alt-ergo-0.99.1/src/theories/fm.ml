  ...
  (P : EXTENDED_Polynome with type r = X.r) = struct
    ...
    module Inequation = struct
      type t = { 
        ple0 : P.t; 
        is_le : bool;
        dep : (Literal.LT.t * Q.t * P.t * bool) list;
        expl : Explanation.t
      }
      ...
      let create p1 p2 is_le a expl = 
        ...
        { ple0 = p; is_le = is_le; dep = [a, Q.one, p, is_le]; expl = expl }
    ...
    let mult_bornes_vars vars monomes ty=
	  ...
	  in
	  I.mult ui (I.power n ui')
        ) (I.point Q.one ty Explanation.empty) vars 
    ...
    let rec add_monome are_eq expl use_x env x =
      try 
        ...
        { env 
          with monomes = MX.add x (u, SX.union old_use_x use_x) env.monomes }
      with Not_found -> 
    ...
    let update_polynomes ?(force=false) env expl =
	  ...
	  if force || I.is_strict_smaller i ip then
    ...
    let mult_list c = 
      List.map (fun (a, coef, p, is_le) -> (a, Q.mult coef c, p, is_le))
    ...
    let assume env uf la =
	  ...
	  (fun (env, eqs, new_ineqs, expl) (a, root, e) ->
	    ...
	    try
		  ...
		  let env =
		    init_monomes
                      are_eq env ineq.Inequation.ple0 SX.empty expl in
		  let env =
		    update_ple0
                      are_eq env ineq.Inequation.ple0 (is_le n) expl in
