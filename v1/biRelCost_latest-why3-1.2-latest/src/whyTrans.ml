...
(* mk_term
    (Tconst
       Number.(ConstInt { ic_negative = false ;
                          ic_abs = int_literal_dec (string_of_int s) }))
 *)
...
let why3_float f =
  ...
  T.t_const (  Why3.Number.ConstReal 
       ...
       (* Number.(ConstReal { rc_negative = false ;
                          rc_abs = real_const_dec is fs None}))  *)
  (* T.t_const (  Why3.Number.(ConstReal { rc_negative : false; rc_abs :   Why3.Number.ConstReal (Why3.Number.real_const_dec is fs None)}) *)
