...
let main_info    fi = SE.message 4 General fi
...
and infer_app m i eP2 eP =
    main_info dp "inf app: @\n@[%a@]@\n@[%a@]" Print.pp_expr (fst eP2) Print.pp_expr (fst eP) ; 
...
and check_fix (i: info) (vi_f : var_info) (vi_x : var_info) (eP: expr * expr) (bty : ty) =
  get_heur >>>= 
    fun heur ->
      ...
      main_info dp "check fix: @\n@[%a@]@  \n" Print.pp_expr (fst eP)  ; 
...
and check_no_change_types (eP: expr * expr) : constr checker =
  fun(ctx, k) ->
  main_info dp "check no changes : @\n@[%a@]" Print.pp_expr (fst eP)  ;
