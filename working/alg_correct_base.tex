...
\\
Unfolding $\eventdep^{val}_{\trace}(\event_1, \event_2, \cdot \event_1 \cdot \event_2, c, D)$, \wqside{typo? DEP$\tau$} we have:
\begin{equation}
...
\label{eq:valdep_inv1}
  \begin{array}{l}   
\config{{c}, \vtrace_0} \rightarrow^{*} 
\config{[\assign{\pi_1(\event_1)}{\expr_1 / \query(\qexpr_1)}]^{\pi_2(\event_1)} ; {c}_1, \vtrace_0 \cdot \vtrace_1}  
...
%
%
By Induction on the operational semantics rules on following execution from Eq.~\ref{eq:valdep_inv1}: \wqside{Surprised we do induction here:-)}
 %
 \[\config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1}
  \rightarrow^{*} 
  \config{[\assign{{x}_2}{\expr_2 / \query(\qexpr_2)}]^{l_2};{c}_2, 
