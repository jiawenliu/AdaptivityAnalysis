...
%
\begin{case}
($\event_1', \event_2' \in \eventset$, $\trace_{ih} \in \mathcal{T}, \trace = \event_1' \tracecat \trace_{ih} \cdot \event_2' \land \trace_{ih} \neq \cdot$)
\\
...
\eventdep(\event_1, \event_2, \event_1 \tracecat \trace_{ih} \cdot \event_2, {c}, D) 
\]
Since $l_e$ has the form $l_e = [\event_1, \cdots, \event_2] $, let $l_e = [\event_1, \event_{ih}] ++ l^{ih}_e ++ [\event_2]$ 
for some $l^{ih}_e \in List(\eventset)$.
\\
By definition of $\uplus$, we know $\eventdep^{val}(\event_1, \event_{ih}, \trace[\event_1:\event_{ih}], c, D)$.
\\
By induction hypothesis on $[\event_{ih}] ++ l^{ih}_e ++ [\event_2] \in \kw{dfs}(\trace[\event_{ih}:\event_2], c, D)$, we know:
$\eventdep^{val}(\event_1, \event_{ih}, \trace[\event_1:\event_{ih}], c, D)$.
\\
Then by the \emph{Value May-Dependent} Definition~\ref{def:event_valdep} and \emph{Event May-Dependent} Definition~\ref{def:event_dep}, we know:
\[
  \eventdep(\event_1, \event_2, \event_1 \tracecat \trace_{ih} \cdot \event_2, {c}, D) 
  \]
