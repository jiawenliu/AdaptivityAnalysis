...
&
\env(\vtrace \cdot (x, \qval, v, l, n)) x \triangleq v
\\
...
\[
\begin{array}{ll}
\env(\vtrace \cdot (x, v, l, n)) x \triangleq \env(\vtrace) x
&
\env(\vtrace \cdot (x, \qval, v, l, n)) x \triangleq \qval
\\
\env(\vtrace \cdot (y, v, l, n)) x \triangleq \env(\vtrace) x
&
\env() x \triangleq \bot
...
\[
\begin{array}{ll}
\pi_v((x, v, l, n)) \triangleq v
&
\pi_v (x, \qval, v, l, n) \triangleq v
\\
\pi_v (b, v, l, n)  \triangleq v
&
\end{array}
\]
%
...
\[
\begin{array}{ll}
\pi_{q} (x, v, l, n) \triangleq v
&
\pi_{q} (x, \qval, v, l, n) \triangleq \qval
\\
\pi_{q} (b, v, l, n)  \triangleq v
&
\end{array}
\]%
% 
Event Action : $\pi_{\action} : \eventset \to \mathcal{VAL}$
\[
\begin{array}{ll}
\pi_{\action} (x, v, l, n) \triangleq (x, l, n)
&
\pi_{\action} (x, \qval, v, l, n) \triangleq (x, \query, l, n)
\\
\pi_{\action} (b, v, l, n)  \triangleq (b, l, n)
&
\end{array}
\]%
%
...
\jl{
  ...
  \vcounter'(l) = \vcounter(l) + 1
  \and 
  \event = (\ssa{x}, \qval, l, \vcounter'(l))
  }
  {
  \config{\ssa{m, [\assign{x}{\query(\qexpr)}]^l, \vtrace, \vcounter}}
  \xrightarrow{} 
  \config{\ssa{m[v/ x], \eskip,  \vtrace \cdot \event, \vcounter'} }
  }
  ~\textbf{ssa-query}
  }
  %
  ...
  \inferrule
  {
  \config{\ssa{c_1, \vtrace, \vcounter}}
  \xrightarrow{}
  \config{\ssa{c_1',  \vtrace', \vcounter'}}
  }
  {
  \config{\ssa{c_1; c_2, \vtrace, \vcounter}} 
  \xrightarrow{} 
  \config{\ssa{c_1'; c_2, \vtrace', \vcounter'}}
  }
  ~\textbf{ssa-seq1}
  ...
  \inferrule
  {
    \config{\ssa{c_2, \vtrace, \vcounter}}
  \xrightarrow{}
  \config{\ssa{c_2',  \vtrace', \vcounter'}}
  }
  {
  \config{\ssa{[\eskip]^{l} ; c_2, \vtrace, \vcounter}} \xrightarrow{} \config{\ssa{ c_2', \vtrace', \vcounter'}}
  }
  ~\textbf{ssa-seq2}
  %
  \and
...
\event_1 \eventeq \event_2
 \triangleq (l_1 = l_2 \land  n_1 = n_2 \land 
 \qval_1 =_q \qval_2) 
\]
%
...
%
$\eventdep(\event_1, \event_2, c, D)$ inductively on form of $\event_1$. 
%
\[
\begin{array}{ll}
\begin{array}{l}
...
\\ 
 \config{c_1, \vtrace_0 \vtrace_1 \cdot \event_1, \vcounter_1'} 
  \qquad \rightarrow^{*} 
  \config{[\assign{\ssa{x}_2}{\expr_2 ~ or ~ \query(\qexpr_2)}]^{l_2} or
  \eif([\sbexpr]^l_2, \cdots) or \ewhile [\sbexpr]^l_2 \cdots; \ssa{c}_2, \vtrace_0 \vtrace_1 \cdot \event_1\vtrace_2, \vcounter_2} 
  \\
...
\pi_{q}(\event_2) \neq \pi_{q}(\event_2') 
\bigwedge
\pi_{\action}(\event_2) = \pi_{\action}(\event_2)
\end{array}
\right)
\end{array} 
&
...
\\ 
 ...
 \config{c_1, \vtrace_0 \vtrace_1 \cdot \event_1', \vcounter_1} 
  \qquad \rightarrow^{*} 
  \config{\ssa{c}_2,  \vtrace_0 \vtrace_1 \cdot \event_1 \vtrace_2', \vcounter_3'} 
\\
\bigwedge
(\event_2) \notin \vtrace_2'
\end{array}
