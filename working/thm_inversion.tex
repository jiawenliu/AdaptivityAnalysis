...
\begin{lem}[Arithmetic Inversion Generalization]
	...
	\[
		...
		\config{\trace, \expr} \aarrow v \land \config{\trace', \expr} \aarrow v' \land v = v'
		\\ \quad
		\implies 
...
\]
%
Let $\diff_{\eventset}$ be a subset of the events in $\trace_2$, satisfying: \todo{refine the notation}
\[
	\forall \event_z \in \eventset^{\asn} \st 
	\event \in \diff_{\eventset} \Leftrightarrow \env(\trace_2) (\pi_1(\event)) \neq \env(\trace_2') (\pi_1(\event))
\]
%
Taking arbitrary $\event_z \in \diff_{\eventset}$, we know
\todo{$\eventdep^{val}(\event_1, \event_z, \trace[\event_1:\event_z], c, D)$}.
\\
From the hypothesis, we know $\neg \eventdep^{val}(\event_z, \event_2, \trace[\event_z:\event_2], c, D) $.
\\
Then by $\eventdep^{val}$ definition, $\forall \event_z', \event_2'', \trace_z',\trace_z,  \trace_1', c_z$ satisfying following two executions, we have $\event_2 \eventeq \event_2''$.
\[
	\begin{array}{l}
		\config{c, \trace_0} \rightarrow^{*}
...
\]
%
Then we know \todo{type correctness}
\[
	\forall z^j \in (\lvar \setminus \diff_{\eventset} ), \trace, \trace' \in \mathcal{T}, v, v' \st 
	\env(\trace) z = \env(\trace') z 
	\land 
...
\]
%
Since $\diff(\event_2, \event_2')$, we know:
%
\[	
