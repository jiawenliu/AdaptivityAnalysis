...
\\
by definition of $\mathcal{A}$, we know either:
\\
...
we know only the lists of events from $\kw{dfs}(\trace, c, D)$ satisfying following restriction are preserved,
\[
	l = \event_1 :: l' ++ [\event_2] 
\]
%
Then we know $(\kw{setfilter} ~(\efun l \to l = \event_1 :: l' ++ [\event_2]) ~ \kw{dfs}(\trace, c, D)) \neq \emptyset \implies$ 
%
\[
\forall l_e \in (\kw{setfilter} ~(\efun l \to l = \event_1 :: l' ++ [\event_2]) ~ \kw{dfs}(\trace, c, D)), \exists l_e'
\st l_e = \event_1 :: l_e' ++ [\event_2] 
\]
%
By $\event_1, \event_2 \in \eventset^{\asn}$, 
we know the $(\efilter (\efun \event \to  \event \in \eventset^{\asn})) $ function preserves $\event_1$ and $\event_2$ for every 
$l_e = \event_1 :: l_e' ++ [\event_2] 
\in (\kw{setfilter} ~ (\efun l \to l = \event_1 :: l' ++ [\event_2]) ~ \kw{dfs}(\trace, c, D))$.
\\
Let $l_{ef} = (\efilter (\efun \event \to  \event \in \eventset^{\asn})) l_e$ for every $l_e = \event_1 :: l_e' ++ [\event_2] 
\in (\kw{setfilter} ~ (\efun l \to l = \event_1 :: l' ++ [\event_2]) ~ \kw{dfs}(\trace, c, D))$, 
we know for every $l_{ef}$:
\[
	\exists l_{ef}' \st l_{ef} = \event_1 :: l_{ef}' ++ [\event_2]
\]
Then, by applying the function
$\emap (\efun  \event \to \pi_1(\event)^{\pi_2(\event)})$ to every 
$l_{ef}$, it is transformed into $l$ as follows:
\[
	l = \pi_1(\event_1)^{\pi_2(\event_1)} :: l' ++ [\pi_1(\event_2)^{\pi_2(\event_2)}]
\]
%
and $l \in \mathcal{A}(\event_1, \event_2, \trace, c, D)$.
%
\\
%
Then we know $\mathcal{A}(\event_1, \event_2, \trace, c, D) \neq \emptyset$ and every $l \in \mathcal{A}(\event_1, \event_2, \trace, c, D)$ satisfies:
\[
