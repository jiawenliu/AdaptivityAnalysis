\documentclass[a4paper,11pt]{article}
\usepackage[table]{xcolor}



\input{prelude}
\input{ldefs}

\usepackage{tikz}
\usetikzlibrary{shapes,arrows}
\newcommand{\THESYSTEM}{\textsf{AdaptFun}}

% Define block styles
\tikzstyle{decision} = [diamond, draw, fill=blue!20, 
    text width=4.5em, text badly centered, node distance=3cm, inner sep=0pt]
\tikzstyle{block} = [rectangle, draw, fill=blue!20, 
    text width=5em, text centered, rounded corners, minimum height=4em]
\tikzstyle{line} = [draw, -latex']
\tikzstyle{cloud} = [draw, ellipse,fill=red!20, node distance=3cm,
    minimum height=2em]

\begin{document}
\title{Program Analysis for Adaptivity Analysis}

\author{}

\date{}

\maketitle
%
% \input{example_cousot}
% \clearpage
% %
\section{Labeled Language}
\input{language}
\clearpage
% %
\section{Event and Trace}
\input{event}
\input{trace}
\clearpage
% %
\section{Dependency and Adpativity}
\input{dep}
\input{adapt}
% \input{dep_example}
\clearpage
% % 
% \section{\THESYSTEM}
% \input{adaptfun}
% % % % 

\clearpage
\section{$\flowsto$ Searching Algorithm}
\input{dep_flowsto_alg}

% \wq{dfs: Trace -> command -> DB -> trace set?  May we can a few sentences to explain it. It seems useful:-) }
% \jl{added types for them}
% \wq{setfilter returns a set of trace? (fun l -> ....): trace -> trace?  So the top level is: setmap (function f: trace -> trace) (set of trace), setmap applies the function to the input set of traces and returns a set of traces. }

% \wqside{I can see this algirithm is over approximated, frome case $\event' $ is a test event.}

% \wq{To be honest, soundness to me is that.  If an algorithm A is sound w.r.t to Dep, then we mean for every output of A, say, (x,y) in A implies Dep(x,y). Do you mean that or you mean soundness of DEP w.r.t algorithm instead? }
\jl{the soundness is build from the 2 theorems, 4.1: correctness of alg w.r.t to flows to; 4.2: soundness w.r.t. alg. And the soundness of DEP w.r.t flowsto is combine of the 2 theorems.}
\section{Soundness}
\input{thm_soundness}
\clearpage
%
\section*{Appendix}
\subsection{Proof of the Correctness of $\flowsto$ Searching Algorithm}
\input{alg_correctness}
\subsection{Proof of the Soundness of $\THESYSTEM$}
% \input{adapt_soundness}

% %
\bibliographystyle{plain}
\bibliography{adaptivity.bib}

\end{document}



