...
\\
 ...
 \[
 P(\cdot \event_1 \tracecat \trace_2 \cdot \event_2) =  
 \begin{array}{l}
\forall c
 \st \eventdep^{val}(\event_1, \event_2, c, D) \land 
 (\config{{c}, \vtrace_0} \rightarrow^{*}
 \config{{c}_1, \vtrace_1 \cdot \event_1}  \rightarrow^{*} 
  \config{{c}_2,  \vtrace_1 \cdot \event_1 \cdot \vtrace_2 \cdot \event_2 } )
\\ \quad \implies
...
with the induction case:
 ...
 \lor 
 \big(P(\cdot \event_1 \tracecat \trace_3 \cdot \event) \land P(\event \cdot \event_2)\big)
  \lor 
\big( \exists \event_3 = (z, l, n, v) \in \trace_2' (P(\cdot \event_1 \tracecat \trace_3 \cdot \event_3) \land 
...
By {Inversion Lemma~\ref{lem:inv_b}} of boolean expression evaluation, we have:
 ...
 \]
  This case is proved.
%
\subsubcaseL{$\eventdep^{val}(\event_1, \event')$}.
\\
