...
\\
3. By Lemma~\ref{lem:onestepdep_inv}, conclude the "direct flowsto" relation.
\begin{proof}
	By the Definition~\ref{def:event_dep}, we know there are 2 cases:
	\begin{case}[ value of $\event_2$ is changed in second execution]
	\input{alg_correct_base}
	\end{case}
	\begin{case}[ $\event_2$ doesn't appear in second execution]
		let $\event_b$ be the testing event  in this case,
		1. in the same way of case 1, we can get:
		\[\pi_1(\event_1) \in VAR(\pi_1(\event_b)) \]
		\\
		By definition of case 2, we also know \todo{rewrite into full definition $\eventdep^{\ctl}(\event_b, \event, c, D)$}.
		\\
