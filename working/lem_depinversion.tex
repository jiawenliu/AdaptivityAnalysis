\begin{lem}[The Soundness of $\flowsto$ Analysis w.r.t. Event Dependency with Empty Trace]
...
\begin{proof}
	...
	%
	\caseL{}
		\bf{(the labelled unique assignment command associated to the $\event_2$ 
		is executed but the value assigned to the variable in this event is changed in second execution).}
	\input{base_case_val}
	% \end{case}
	\caseL{}
		\bf{(the labelled unique assignment command associated to the $\event_2$ isn't executed in second execution).}
		\\
		Proof Summary:
		\\
		Let $\event_b$ be the testing event in this case,
		In the same way of case 1, we can get:
	...
	\[		
		...
		\\ \quad \implies	  
			...
			\tlabel_{\trace_3} \cap \tlabel_{\trace_3'} = \emptyset
			  ...
			  \land \event_2 \not\eventin \trace_3'
		\\ \quad \implies	
		\forall z \in VAR(\pi_1(\event_b)) \st \exists l \in \mathbb{N} \st 
		\flowsto(z^l, \pi_1(\event_2)^{\pi_2(\event_2)},)
	\end{array}
	\]
	\end{lem}
	Proof Summary:
...
%  \rightarrow^{*} 
  ...
  \trace_1 \tracecat [\event_1] \tracecat \trace [\event_b]} 
  \\
  \qquad  \rightarrow^{*} 
  \config{[\assign{{x}_2}{\expr_2 / \query(\qexpr_2)}]^{l_2}; c_{3b}', 
  \trace_1 \tracecat [\event_1] \tracecat \trace [\event_b] \tracecat  \trace_{3a}}
  \\ \qquad \rightarrow^{\rname{assn/query}}
  \config{ c_{3b}', 
  \trace_1 \tracecat [\event_1] \tracecat \trace [\event_b] \tracecat  \trace_{3a} \tracecat [\event_2]}
  \rightarrow^{*} 
  \config{c_3, 
  \trace_1 \tracecat [\event_1] \tracecat \trace [\event_b] \tracecat  \trace_{3a} \tracecat [\event_2] \tracecat \trace_{3b}}
  % 
\end{array}
\end{equation}
%  %
, where $\trace_3 = \trace_{3a} \tracecat [\event_2] \tracecat trace_{3b}$,
...
we know:
\\
in case of $\eif ([b]^{l_b}, c_t, c_f)$, $[\assign{{x}_2}{\expr_2 / \query(\qexpr_2)}]^{l_2}$ can only be in 
$c_t$ or $c_f$.
\\
In case of 
	\end{proof}
