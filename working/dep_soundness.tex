...
%
%
 By Evaluation~\ref{eq:valdep_inv1}, we have:
\[
  \begin{array}{l}   
\config{{c}, \vtrace_0} \rightarrow^{*} 
...
\end{array}
 ...
 \[\config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1}
  ...
  \vtrace_0 \cdot \vtrace_1 \cdot \event_1} \]
 %
 we know by \todo{Inversion Lemma}, \todo{notation $[\eskip]{}^*$}:
 \[
 c_1 =_c 
...
by substituting $c_1$ in the execution in Evaluation~\ref{eq:valdep_inv2}, we have 
...
\\
 By substituting $c_1$ in the execution in Evaluation~\ref{eq:valdep_inv2}, we have following possible executions
 %
  \[
  \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1'} 
  \rightarrow^{\eskip^*} 
...
\\
%
By Equation~\ref{eq:valdep_invn}, we know:
%
\[
 \vcounter(\vtrace_2') l_2 = 0
\]
%
Then, we know $\event_2' \notsigin \vtrace_2'$. 
\\
...
%
 \caseL{$m = 1$}
%
Let $\vtrace_2 = {} \cdot {} \event_{ii}$, there are 2 sub-cases: 
%
 \[
...
%
By conclusion from \textbf{case: $m = 0$}, we have following execution based on Evaluation~\ref{eq:valdep_inv1}:
\[
  \begin{array}{l}   
\config{{c}, \vtrace_0} \rightarrow^{*} 
\config{[\assign{{x}_1}{\expr_1 / \query(\qexpr_1)}]^{l_1} ; {c}_1, \vtrace_0 \cdot \vtrace_1}  \rightarrow^{assn/query}
...
and
 ...
 \]
 %
 By substituting $c_1$ in the execution in Evaluation~\ref{eq:valdep_inv2}, and conclusion from \textbf{case: $m = 0$}, we know the only possible execution satisfying:
 \[
  \vcounter(\vtrace_2') l_2 = 0 \land  \vcounter(\vtrace_2') l_' = 0
\]
%
 is as follows:
 %
...
\subcaseL{$\event' \in \eventset^{\test}$}
 %
Let $\event' = (b, l_b, n_b, v_b)$, by Evaluation~\ref{eq:valdep_inv1} and {Inversion Lemma~\ref{lem:inv_test}}, we have:
\[
  \begin{array}{l}   
\config{{c}, \vtrace_0} \rightarrow^{*} 
...
\]
%
By execution in Evaluation~\ref{eq:valdep_inv2} and $\event'' = (b, l_b, n_b, \efalse)$, we have:
 %
  \[
  \begin{array}{l}   
  \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1'} 
...
\]
%
By execution in Evaluation~\ref{eq:valdep_inv2} and $\event'' = (b, l_b, n_b, \etrue)$, we have:
 %
  \[
  \begin{array}{l}   
  \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1'} 
