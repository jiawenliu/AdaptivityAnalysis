...
%   \forall \event_1, \event_2 \in \eventset^{\asn}, 
%   (\pi_1(\event_1), \pi_2(\event_1) ),
%     (\pi_1(\event_2), \pi_2(\event_2) ) \in \lvar_c \st
...
\forall \trace \in \mathcal{T} \st
  ...
  \land \cdots \land \flowsto(z_n^{r_n}, (\pi_1(\event_2)^{ \pi_2(\event_2)} ), c) \Big)
\end{array}
\]
\end{thm}
...
\begin{array}{l}
  ...
  | l' :: \event' =>  
  \\ \quad \qquad
  \eif \eventdep^{val}(\event', \event, c) 
  \ethen \{l::\event\}
  \eelse l' \uplus \event
\end{array}
\]
...
\begin{array}{l}
  ...
  = 
  \bigcup\limits_{l \in \kw{dfs}(\trace, c, D) \land l.end = \event_2}
  \emap \event => \pi_1(\event)^{\pi_2(\event)} ~ l
   \\ \qquad 
   \ecase \event' \in \eventset^{\test} \land \event_2 \in body(\event'):
   \\ \qquad \quad 
    \bigcup\limits_{\event'', \trace_h, \trace_t \st 
     \trace_h \cdot \event'' \tracecat \trace_t = \trace'
     \land \pi_1(\event'') \in VAR(b)  \land 
     \pi_2(\event'') = \llabel(\trace') \pi_1(\event'')}
     \\ \qquad \qquad \qquad
      \mathcal{A}(\event_1, \event'', \trace_h, c, D) \uplus [\pi_1(\event'')^{\pi_2(\event'')}, \pi_1(\event_2)^{\pi_2(\event_2)}]  \\ \qquad   
   \ecase \event' \in \eventset^{\test}
   \land \event_2 \notin body(\event'):
   \\ \qquad \quad 
   \bigcup_{\event_b, \trace_h', \trace_t' \st 
   \trace_h' \cdot \event_b  \tracecat \trace_t' = \trace_2' \land 
   \pi_1(\event_b)^{\pi_2(\event_b)} \in \lvar_{body}}
   \\ \qquad \quad 
   \bigcup_{\event'', \trace_h, \trace_t \vert 
     \trace_h \cdot \event'' \tracecat \trace_t = \trace'
     \land
     \pi_1(\event'') \in VAR(b) 
     \land 
     \pi_2(\event'') = \llabel(\trace') \pi_1(\event'')}
     \\ \qquad \qquad \qquad
      \mathcal{A}(\event_1, \event'', \trace_h, c, D) \uplus [\pi_1(\event'')^{\pi_2(\event'')}, \pi_1(\event_b)^{\pi_2(\event_b)}] \uplus \mathcal{A}(\event_b, \event_2, \trace_t', c, D)
     \\ \qquad \quad
     
     \bigcup_{\trace_h, \trace_t, \event'' \st \trace' = \trace_1 \cdot \event'' \tracecat \trace_t}
     \mathcal{A}(\event_1, \event'', \trace_h, c, D) \uplus [\pi_1(\event'')^{\pi_2(\event'')}, \pi_1(\event_2)^{\pi_2(\event_2)}]
\end{array}
\]
