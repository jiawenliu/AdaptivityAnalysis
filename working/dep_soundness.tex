...
\config{{c}_1, \vtrace_1 \cdot \event_1}  \rightarrow^{*} 
  \config{{c}_2,  \vtrace_1 \cdot \event_1 \cdot \vtrace_2 \cdot \event_2 } ~(1)
  % 
 \\ 
 \bigwedge &
  \config{{c}_1, \vtrace_1 \cdot \event_1'}  \rightarrow^{*} 
  \config{{c}_2',  \vtrace_1 \cdot \event_1'  \cdot \vtrace_2' \cdot \event_2' } ~ (2)
\\
...
By Inversion Lemma~\ref{lem:inv_asn} on 
 $\event_1$, $\event_2$ and execution $(1)$, we know $\exists \expr_1 or \qexpr_1$, $\exists \expr_2 or \qexpr_2$
 \[
  \begin{array}{l}   
\config{{c}, \vtrace_0} \rightarrow^{*} 
...
\rightarrow^{assn/query}
 \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1} \\
  ...
  % 
\end{array} ~ (\mbox{inv}_1)
 \]
%
%
By Inversion Lemma~\ref{lem:inv_asn} on 
 $\event_1'$, $\event_2'$ and execution $(2)$, we know $\exists \expr_1' or \qexpr_1'$, $\exists \expr_2' or \qexpr_2'$
 \[
  \begin{array}{l}   
  \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1'} 
  \rightarrow^{*} 
...
\] 
%
By $\event_2 \sigeq \event_2'$, 
we know
\[
  x_2' = x_2 \land l_2' = l_2 \land 
  \vcounter(\vtrace_0 \cdot \vtrace_1 \cdot \event_1 \cdot \vtrace_2 \cdot \event_2) l_2 = 
  \vcounter(\vtrace_0 \cdot \vtrace_1 \cdot \event_1' \cdot \vtrace_2' \cdot \event_2') l_2 ~ (\mbox{inv}_d)
\] 
Then, we have:
  \[
  \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1'} 
...
%
%
 By evaluation $(\mbox{inv}_1)$, we have:
\[
  \begin{array}{l}   
\config{{c}, \vtrace_0} \rightarrow^{*} 
...
\end{array}
 ...
 \]
 %
 By substituting $c_1$ in the execution in $(\mbox{inv}_2)$, we have 
 %
  \[
  \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1'} 
  \rightarrow^{\eskip^*} 
...
%
By conclusion from \textbf{case: $m = 0$}, we have following execution based on $(\mbox{inv}_1)$:
\[
  \begin{array}{l}   
\config{{c}, \vtrace_0} \rightarrow^{*} 
\config{[\assign{{x}_1}{\expr_1 / \query(\qexpr_1)}]^{l_1} ; {c}_1, \vtrace_0 \cdot \vtrace_1}  \rightarrow^{assn/query}
...
we have the same 2 executions as in $(\mbox{m1asn}_2)$ and $(\mbox{m1asn}_1)$.
\\
...
\subcaseL{$\event' \in \eventset^{\test}$}
 %
Let $\event' = (b, l_b, n_b, v_b)$, by evaluation $\mbox{inv}_1$ and {Inversion Lemma~\ref{lem:inv_test}}, we have:
\[
  \begin{array}{l}   
\config{{c}, \vtrace_0} \rightarrow^{*} 
...
\end{array}
 \]
By $(\mbox{inv}_d)$, we know:
 \[
 \vcounter(\vtrace_0 \cdot \vtrace_1 \cdot \event_1 \cdot \event' \cdot \event_2) l_2 = 
  \vcounter(\vtrace_0 \cdot \vtrace_1 \cdot \event_1' \cdot \event'' \cdot \trace_2' \cdot \event_2') l_2 
 \]
 %
 Unfolding the $\vcounter$, we have:
  \[
 \vcounter(\vtrace_0 \cdot \vtrace_1) l_2 + (l_1 = l_2) + 1 = 
  \vcounter(\vtrace_0 \cdot \vtrace_1) l_2 + (l_1 = l_2) + \vcounter(\vtrace_2') l_2  + 1
 \]
 %
 i.e.,
 \[
 ...
 \]
Consider the two subcases: $\eventdep^{val}(\event_1, \event', c, D)$ or $\neg\eventdep^{val}(\event_1, \event', c, D)$
%
\subsubcaseL{$\neg\eventdep^{val}(\event_1, \event', c, D)$}
By execution in $(\mbox{m1test}_2)$ and $(\mbox{m1test}_1)$, since $\neg\eventdep^{val}(\event_1, \event', c, D)$, we have:
\[
  \event' \eventeq \event''
\]
...
\]
%
Substituting $c_3'$ by $c_3$ in $(\mbox{m1test}_2)$, we have:
\[
  \config{c_3', \vtrace_0 \cdot \vtrace_1 \cdot \event_1' \cdot \event''} 
  \rightarrow^{\eskip*} 
  \config{[\assign{{x}_2}{\expr_2 / \query(\qexpr_2)}]^{l_2};{c}_4, 
...
%
\subsubcaseL{$\eventdep^{val}(\event_1, \event', c, D)$}
By execution in $(\mbox{m1test}_2)$ and $(\mbox{m1test}_1)$, since $\eventdep^{val}(\event_1, \event', c, D)$, we have:
\[
  \event' \eventeq \event'' \lor \sigdiff(\event', \event'')
\]
%
Case of $\event' \eventeq \event''$ is proved in \textbf{subsubcase:} $\neg \eventdep^{val}(\event_1, \event', c, D)$.
\\
...
\]
\begin{subproof}[\textbf{Subsubsubcase}:] $c_1 =_c \eif ([b]^{l_b}, c_t, c_f);{c}_2$ 
\\
Without loss of generalization, let $\event' = (b, l_b, n_b, \etrue)$ and
$\event'' = (b, l_b, n_b, \efalse)$,
by execution $(\mbox{m1test}_1)$ and $\event' = (b, l_b, n_b, \etrue)$, we have:
\[
  \begin{array}{l}   
  \config{{c}, \vtrace_0} \rightarrow^{\eskip^*} 
  \config{[\assign{{x}_1}{\expr_1 / \query(\qexpr_1)}]^{l_1} ; {c}_1, \vtrace_0 \vtrace_1}  
  \rightarrow^{assn/query}
  \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1} 
  \\
  \qquad \rightarrow^{*} 
  \config{\eif ([b]^{l_b}, c_t, c_f);{c}_2, 
  \vtrace_0 \vtrace_1 \cdot \event_1} 
  \rightarrow^{if-t} 
...
\]
%
By execution $(\mbox{inv}_2)$ and $\event'' = (b, l_b, n_b, \efalse)$, we have:
 %
  \[
  \begin{array}{l}   
  \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1'} 
...
~\mbox{(m1testif)}_2
 \]
 %
 By uniqueness of label, we have three cases:
 \begin{enumerate}
 ...
 \]
 This is contradict to the existence of execution in $\mbox{(m1testif)}_2$.
 \\
...
\]
Substituting $c_2$ in the execution in $\mbox{(m1testif)}_2$, we have:
\[
    \begin{array}{l}   
  \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1'} 
  \rightarrow^{\eskip^*} 
...
\[
  \forall z \in \lvar \setminus \{x_1^{l_1}\} \cup VAR{\trace_2'} \st
...
we have:
\[
  x_1^{l_1} \in VAR(\expr_2) \bigvee_{z_i^{r_i} \in VAR{\trace_2'}} z_i^{r_i} \in VAR(\expr_2)
\]
%
By $\flowsto$ definition, we have:
\[
   \flowsto(x_1^{l_1}, x_2^{l_2}) \bigvee_{z_i^{r_i} \in VAR{\trace_2'}} \flowsto(z_i^{r_i}, x_2^{l_2})
\]
%
By $ \bigwedge_{\forall z_i^{r_i} \in \lvar_{c_f}} \flowsto(x_1^{l_1}, z_i^{r_i})$ and $VAR{\trace_2'} \subseteq \lvar_{c_f}$, we have:
\[
\flowsto(x_1^{l_1}, x_2^{l_2}) \bigvee_{z_i^{r_i} \in VAR{\trace_2'}} 
...
\end{subproof}
%
\begin{subproof}[\textbf{Subsubsubcase}:] $c_1 =_c \ewhile [b]^{l_b} \edo c_w;{c}_2$ 
%
\\
There are 2 cases:
$\event' = (b, l_b, n_b, \etrue) \land \event'' = (b, l_b, n_b, \efalse)$,
or
$\event' = (b, l_b, n_b, \efalse) \land \event'' = (b, l_b, n_b, \etrue)$.
\\
\textbf{subsubsubsubcase:} $\event' = (b, l_b, n_b, \etrue) \land \event'' = (b, l_b, n_b, \efalse)$
\\
By execution $(\mbox{m1test}_1)$ and $\event' = (b, l_b, n_b, \etrue)$, we have:
\[
  \begin{array}{l}   
...
and 
\[
  c_w;\ewhile [b]^{l_b} \edo c_w; c_2 =_c [\assign{{x}_2}{\expr_2 / \query(\qexpr_2)}]^{l_2};{c}_4
\]
%
By execution $(\mbox{inv}_2)$ and $\event'' = (b, l_b, n_b, \efalse)$, we have:
 %
...
%
 \end{enumerate}
%
\textbf{subsubsubsubcase:}$\event' = (b, l_b, n_b, \efalse) \land \event'' = (b, l_b, n_b, \etrue)$
\\
By execution $(\mbox{m1test}_1)$ and $\event' = (b, l_b, n_b, \etrue)$, we have:
\[
  \begin{array}{l}   
...
\]
%
By execution $(\mbox{inv}_2)$ and $\event'' = (b, l_b, n_b, \etrue)$, we have:
 %
  \[
  \begin{array}{l}   
  \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1'} 
...
\[
  ...
  \\
  \qquad \rightarrow^{*} 
  \config{c_2,\vtrace_0 \cdot \vtrace_1 \cdot \event_1' \cdot \event'' \cdot \trace_2'}
  \\
  \qquad \rightarrow^{\eskip*} 
  \config{[\assign{{x}_2}{\expr_2 / \query(\qexpr_2)}]^{l_2};{c}_4, 
  \vtrace_0 \cdot \vtrace_1 \cdot \event_1' \cdot \event'' \cdot \trace_2'}
...
\]
%
By $ \bigwedge_{\forall z_i^{r_i} \in \lvar_{c_w}} \flowsto(x_1^{l_1}, z_i^{r_i})$ and $VAR{\trace_2'} \subseteq \lvar_{c_f}$, we have:
\[
\flowsto(x_1^{l_1}, x_2^{l_2}) \bigvee_{z_i^{r_i} \in VAR{\trace_2'}} 
(\flowsto(x_1^{l_1}, z_i^{r_i}) \land \flowsto(z_i^{r_i}, x_2^{l_2}))
\]
