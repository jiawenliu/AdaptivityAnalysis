...
%   \flowsto(z^l, {x}_2^{l_2}, c))\big)
 \end{array}
  ...
  \todo{
  intuition of
  $P(\cdot \event_1 \tracecat \trace_{ih} \cdot \event_2, c')$,
  $\event_{ih}$ doesn't have dependency on $\event_1$,
  $c'$ is a program without generating $\event_{ih}$.
  then flowsto relation from $x_1$ to $x_2$ in $c'$ is exactly the same as  flowsto relation from $x_1$ to $x_2$ in $c$.
  }
\\
%
