...
%
 %
 \todo{By induction on structure of $\vtrace_2$, there are 2 induction cases:
 %
\\
$\trace_2 = \cdot$
\\
...
%
\\
 It is sufficient to show $\forall c$: 
 \[
  \begin{array}{l}
P(\cdot \event_1 \tracecat \trace_2 \cdot \event_2, c) \triangleq  
\eventdep^{val}(\event_1, \event_2, c, D) \land 
 (\config{{c}, \vtrace_0} \rightarrow^{*}
 \config{{c}_1, \vtrace_1 \cdot \event_1}  \rightarrow^{*} 
  \config{{c}_2,  \vtrace_1 \cdot \event_1 \cdot \vtrace_2 \cdot \event_2 } )
\\ \quad \implies
...
\]
  %
with the induction case:
 $$
\forall c \st  P(\cdot \event_1 \tracecat \trace_{ih} \cdot \event_{ih} \cdot \event_2, c).
  $$
}
%
The induction case is proved as follows: (induction hypothesis)
 $$
   \begin{array}{l}
P(\cdot \event_1 \tracecat \trace_{ih} \cdot \event_{ih} \cdot \event_2, c)
 ...
 \big(P(\cdot \event_1 \tracecat \trace_{ih} \cdot \event_{ih}, c) \land P(\event_{ih} \cdot \event_2, c)\big) 
  \\ \qquad \lor 
 \Big(\exists \event_i = (z_i, l_i, n_i, v_i) \eventin \trace_{ih} \st (P(\cdot \event_1 \tracecat \trace_{i} \event_i, c) 
 \land \flowsto(z_i, x_2^{l_2}, c) \Big)
%   \lor 
...
%
\begin{subcase}[$\event_{ih} \in \eventset^{\asn}$].
...
%
\begin{subsubcase}[$\big(\eventdep^{val}(\event_1, \event_{ih}, c, D) \land 
\eventdep^{val}(\event_{ih}, \event_2, c, D)\big)$]
\label{case:valdep_ihasndep}.
\\
 %
By induction hypothesis from trace $\event_1  \tracecat \trace_{ih} \cdot \event_{ih}$, $\eventdep^{val}(\event_1, \event_{ih}, c,D)$, and following execution from Eq.~\ref{eq:m1asn_inv1}:
\[
\config{{c}, \vtrace_0} \rightarrow^{*} 
 \config{c_1, \vtrace_0 \cdot \vtrace_1 \cdot \event_1}
  \rightarrow^{*} 
...
\]
%
By induction hypothesis from Case.~\ref{case:valdep_base}, on trace $\event_{ih} \cdot \event_2$ and $\eventdep^{val}(\event_{ih}, \event_2, c, D)$, , and following execution from Eq.~\ref{eq:m1asn_inv1}:
\[
\config{{c}, \vtrace_0} \rightarrow^{*} 
  \config{{c}_2,  \vtrace_0 \cdot \vtrace_1 \cdot \event_1 \tracecat \trace_{ih} \cdot \event_{ih}}
\rightarrow^{*} 
...
%
\begin{subsubcase}[{$\neg \eventdep^{val}(\event_1, \event_{ih}, c, D) 
\lor \neg \eventdep^{val}(\event_{ih}, \event_2, c, D)$}].
%
\begin{subsubsubcase}[$\neg \eventdep^{val}(\event_1, \event_{ih}, c, D) $].
\\
...
\end{array}
 ...
 \]
 %
 By \todo{inversion Lemma of deterministic of the program evaluation}, we know:
 %
 \[
...
%
%
\todo{Let $\eventset_{diff} \subseteq \eventset $ be all the events differ in $\trace_{ih}$ and $\trace_{ih}'$.}
%
\\
...
% %
% \\
% By $\neg \eventdep^{val}(\event_1, \event_{ih}, c, D)$, we know:
% \[
...
%   \env(\vtrace_0 \cdot \vtrace_1 \cdot \event_1' \cdot \event_{ih}') (u) =  
%   \env(\vtrace_0 \cdot \vtrace_1 \cdot \event_1 \cdot \event_{ih}) (u)
% \]
% %
...
% %
% \\
% Then we know $x_1 \neq z$ and $\llabel(\vtrace_0 \cdot \vtrace_1 \cdot \event_1' \cdot \event_{ih}') x_1 = l_1$.
% \\
% %
% By {Inversion Lemma~\ref{lem:inv_a}} of arithmetic expression evaluation, Inversion Lemma~\ref{lem:inv_live}, we have 
% \[
...
\todo{
need to rewrite: 
rewrite from proof of case $\trace_2 = \event_{ih}$  into proof of case
$\trace_2 = \trace_{ih} \cdot \event_{ih}$ 
}
\begin{subcase}[{$\event_{ih} \in \eventset^{\test}$}].
...
\end{subsubcase}
%
\begin{subsubcase}[{$\eventdep^{val}(\event_1, \event_{ih}, c, D)$}].
