...
\]
%
Trace Label Set $\tlabel : \mathcal{T} \to \mathcal{P}{(\mathbb{N})}$ 
The label of the lastest assignment event which assigns value to variable $x$.
\[
  \begin{array}{llll}
\tlabel_{(\trace' \cdot (x, l, v) )} \triangleq \{l\} \cup \tlabel_{(\trace' )}
&
\tlabel_{(\trace' \cdot (b, l, v))} \triangleq \{l\} \cup \tlabel_{(\trace' )}
&
\tlabel_{(\trace' \cdot (x, l, \qval, v))} \triangleq \{l\} \cup \tlabel_{(\trace' )}
&
\tlabel_{(\cdot)} \triangleq \{\}
\end{array}
\]
...
%
%
\todo{Need to Consider The Necessary}
\\
...
\land \trace_1 \cdot \event_1' \cdot \trace_2 \cdot \event_2' \cdot \trace_3 = \trace_p$
\end{itemize}
\end{defn}
%
%
...
\[
\forall \trace \in \mathcal{T}, c \st
\config{c, \vtrace} \to^{*} \config{\eskip, \trace \cdot \trace'} \land \trace_p = p(\trace')
\implies
\trace_p\in \mathcal{T}
\] 
