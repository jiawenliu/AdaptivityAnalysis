...
\begin{array}{llll}
%
\mbox{remove: While Map} 
& w & ::= & [] ~|~  w[l \to n]
\\
%
\mbox{remove: Annotated Query} 
& \aq & ::= & (\qval, l , w )
\\
%
\mbox{Trace} & t_{(\ssa{c, m}, D)} 
...
Given a program $\ssa{c}$, its assigned variables $\avar$ is a vector containing all variables newly assigned in the program preserving the order, $\avar \subset \mathcal{SVAR}$.
...
\\
Given a program $\ssa{c}$, its query variables $\qvar$ is a vector containing all variables newly assigned by a query in the programm preserving the order, $\qvar \subset \mathcal{SVAR}$.
It is defined as follows:
$$
  ...
  \begin{array}{ll}
     	...
     	& \ssa{c} = [\ssa{\assign x \query(\qexpr)}]^{(l, w)} 
     	\\
     	\qvar_{\ssa{c_1}} ++ \avar_{\ssa{c_2}} 	
     	& \ssa{c} = \ssa{c_1};\ssa{c_2}
     	\\
     	\qvar_{\ssa{c_1}} ++ \avar_{\ssa{c_2}} ++ \ssa{[\bar{x}, \bar{y}, \bar{z}]} 
     	& \ssa{c} =\eif([\sbexpr]^{(l, w)} , \ssa{[\bar{x}, \bar{x_2}, \bar{x_2}], 
     	[\bar{y}, \bar{y_2}, \bar{y_3}], 
     	[\bar{z}, \bar{z_2}, \bar{z_3}], c_1, c_2}) 
     	\\
...
\\
	\begin{defn}[Trace $t_{(\ssa{c, m}, D)}$].
	Given a program $\ssa{c}$ with a starting memory $m$ and database $D$, 
	its trace $t_{(\ssa{c, m}, D)} : \mathcal{SVAR} \to \mathbb{N}$ 
	is a function mapping from the program $\ssa{c}$'s assigned variables $x \in \avar_{\ssa{c}}$ 
	to a natural number $n \in \mathbb{N}$.
	This natural number $n$ denotes the variable $\ssa{x}'s$ reaching times in executing the program $\ssa{c}$ 
	\end{defn}
}
%
{%
...
\and
\jl{Memory \times Command  \times Trace \Rightarrow^{}  Memory \times Command  \times Trace}
\\
{
	\inferrule
	{
...
{
\config{\ssa{m, [\assign{\ssa{x}}{\ssa{\query(\qval)}}]^l, t}} 
\xrightarrow{} 
\config{\ssa{ (x \mapsto v)::m, \eskip, t'}}}
~\textbf{ssa-query-v}
}
%
\and
...
{
\config{\ssa{ m, [\assign x v]^{l}, t} } 
\xrightarrow{} 
\config{\ssa{(x \mapsto v)::m, [\eskip]^{l}, t'} }
}
~\textbf{ssa-assn}
}
%
...
one variable $\ssa{x}_1 \in \avar_{\ssa{c}}$ may depend on another variable 
$\ssa{x}_2 \in \avar_{\ssa{c}}$ in $\ssa{c}$ denoted as 
%
...
$\config{\ssa{m}, \ssa{c}, []} 
\to^{*}
\config{\ssa{m'}, \eskip, t_{\ssa{c, m}, D}}$,
...
\begin{array}{rlcl}
	...
	~ \middle\vert ~
	t_{\ssa{c, m}, D} (x) = n, \forall x \in \qvar_{\ssa{c}}
	\right\}
\end{array}
\]
\end{defn}
