...
\begin{example}[Simple Seq]
    ...
    \begin{example}[If with Data-Control Dependency Separated]
        %
        %
        \[
        %
        ...
        \begin{example}[If with Data-Value and Data-Control Dependency Overlapped]
            ...
            \begin{example}[Simple While without Recursive Data-Value Dependency]
                %
                %
                \[
                %
...
%
                \begin{example}[Simple While with Recursive Data-Value Dependency]
                    ...
                    \end{example}
%
                \begin{example}[Nested While with Simple Recursive Data-Value Dependency]
                    %
                    %
                    \[
                    %
                ...
                \todo{Over-Approximated, because we cannot accurately calculate the multi-path reachability bounds.
                But can extend with the analysis method in \cite{Sumit2010rechability} to cover this}
                    %
                    %
                    \[
                    %
                    ...
                    \begin{example}[Simple While with Recursive Multiple-Variable Data-Value Dependency]
                        ...
                        \begin{example}[Nested While with Nested Recursive Multiple Variable 
                            ...
                            \begin{example}[While with Data-Value and Data-Control Dependency Overlapped]
                                    ...
                                    \[
                                    %
                                    \kw{smpWhileValCtl} \triangleq
                                    \begin{array}{l}
                                        \clabel{ \assign{x}{\query(\chi[0])}}^{1} ; \\
                                        \clabel{ \assign{y}{\query(\chi[1])}}^{2} ; \\
                                            \ewhile ~ \clabel{x > 0}^{3} ~ \edo ~ \\
                                            \Big(
                                             \clabel{\assign{x}{\query(\chi[x - 1])} }^{4}  ; \\
                                             \clabel{ \assign{y}{\query(\chi[x] * \chi[y])}}^{5} ; \\
                                            \Big);
                                        \end{array}
                                    \]
                                    \end{example}
                            
                                    \todo{Over approximated, we will not infer the most accurate reachability bound for Variable x}