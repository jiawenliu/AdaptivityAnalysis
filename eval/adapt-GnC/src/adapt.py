...
class Runner():
	...
	def runandplot_with_one_mech(self, q_max_list = [2000], q_adapt_list = [10], mech_name = "GnC_DataSplit"):
		runs = 5
		n_list = [50000]
		beta, tau = 0.05, 1.0
		sigma = 0.03
		...
		def plot(x_list, rmse_list):
			...
			plt.legend()
			plt.grid()
			plt.savefig("../plots/" + mech_name + "-pldi2.png")
			plt.show()
	...
	##################################################################################################################
		...
		f = open('../results/'+ mech_name + "-pldi2.txt", 'w')
	# ###################################### Emperical Result: ######################################
