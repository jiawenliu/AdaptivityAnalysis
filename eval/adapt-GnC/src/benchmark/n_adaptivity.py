...
def n_adaptivity(delta, strategy, mechanism, epoch = MAX_EPOCH):
    l, queried_set = 0, []
...
def eval_n_adaptivity(delta, n = DATA_SIZE, cardinality = CARDINALITY, q_max = MAX_QUERY_NUM, mechanism = mech.Mechanism()):
    ...
    n_adaptivity(delta, strategy, mechanism)
...
Baseline_rmse = np.mean([eval_n_adaptivity(repeated_query_sub_delta, n, dimension, q_max, Baseline) for _ in range(runs)], axis = 0)
...
Thresh_rmse = np.mean([eval_n_adaptivity(repeated_query_sub_delta, n, dimension, q_max, Thresh) for _ in range(runs)], axis = 0)
...
Gauss_rmse = np.mean([eval_n_adaptivity(repeated_query_sub_delta, n, dimension, q_max, Gauss) for _ in range(runs)], axis = 0)
