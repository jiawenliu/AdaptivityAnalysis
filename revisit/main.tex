...
\begin{figure}[h]
 ...
 \begin{array}{rcl}
	...
	\text{} &\quad& T_{query} :: = q \sep \tcaseof{T_b}\ \{ c_i \Rightarrow T_{query_i}\}_{c_i \in b} \\[2mm]
     \text{Depth} &\quad&   \ddep(c) = 0 \\
       &\quad& \ddep(!t) = \ddep(t) \\
           &\quad&      \ddep( \app{t_1}{t_2} ) = \max(\ddep(t_1), \ddep(t_2)) \\
            &\quad&  \ddep(M(t)) = 1 + \ddep(t) \\
             &\quad&  \ddep(\abs{x}{t}) = \ddep(t) \\
              &\quad& \ddep(x) = 0 \\
              & \quad & \ddep(q) = 0 \\
              & \quad & \ddep((t_1, t_2)) = \max(\ddep(t_1), \ddep(t_2))\\
              & \quad & \ddep(\letx{(x_1, x_2)}{t}{t'}) = \max(\ddep(t), \ddep(t'))\\
              & \quad & \ddep(\letx{!x}{t}{t'}) = \max(\ddep(t), \ddep(t'))\\
              & \quad & \ddep(\tcaseof{t}\ \{c_i \Rightarrow t_i\}_{c_i \in b}) = \max(\ddep(t), \ddep(t_i))\\
            & \quad & \ddep(\Lambda.t) = \ddep(t)\\
              & \quad & \ddep(t\, []) = \ddep(t)\\
\end{array}
$$
\caption{syntax}
