...
\begin{figure}
\begin{mathpar}
   \inferrule{  \mathsf{fetch} (\env,x)  =  (\valr, \adapt)  }{x, \env  \bigstep{\adapt} \valr, \env }~\textsf{var}  
  %
  % \and
  % %
 ...
 \inferrule{
  ...
  %
  \inferrule{
     \expr_1, \env \bigstep{\adapt_1} \lambda x.\expr , \env_1 \\
...
\begin{figure}
 ...
 \begin{figure}
  \begin{mathpar}
    ...
    %
    \inferrule{
      \forall \theta', \valr', R'.  \tvdash{\_} (\theta', \valr') : \type_1 
      \tvdash{  \idx \times R' +\nnatA }
      \expr: \type_2
    }{
     \tvdash{0} ( \env, \lambda x. \expr )  : \bang{\idx} \type_1
      \lto{\nnatA} \type_2
    }~\textbf{C-lambda}
    \and
    %
    \inferrule{
       \tvdash{\nnatA_1} (\env_1, \expr_1) :  \bang{\idx} \type_1
      \lto{\nnatA} \type_2      \\
      \tvdash{\nnatA_2} (\env_2, \expr_2): \type_1 
    }{
       \tvdash{    \nnatA_1 +
        \idx \times \nnatA_2 + \nnatA    } ( \env_1 \uplus \env_2 ,  \expr_1 \eapp \expr_2) : \type_2
    }~\textbf{C-app}
    %
    \and
    %
  ...
  \begin{mathpar}
    \begin{array}{lll}
      ...
      \{(\env, \lambda x.\expr, R) ~|~ \forall \valr', \env',
                                                             ...
                                                             \bang{k} \type_1}.\\
      & & 
      \implies (\env[x \mapsto (\valr', \env', R' )],  \expr ) \in
          \lre{    }{ \nnatA }{\type_2}     \} \\
      %
      \\
      %
      \lre{}{\nnatA}{\type} & = & \{  (\env, \expr) ~|~  (\env, \expr
                                  \bigstep{R} \env', \valr ) \\
      & & ~~~~~~~~~~~~~~~~~\Rightarrow R \leq \nnatA \conj 
     (\env', \valr, R) \in \lrv{\type})
      \}
    \end{array}
  \end{mathpar}
 ...
 \begin{thm}[Monotonicity]
  ...
  \begin{enumerate} 
   \item If  $(\env,
     ...
     \expr) \in  \lre{}{\nnatA'}{ \type} $.
   \item   If  $(\env,
     \valr, R) \in  \lrv{\type} $ and $R' \geq R$,  then  $ (\env,
     \valr, R') \in  \lrv{\type} $.
  \end{enumerate}
\end{thm}
