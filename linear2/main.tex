  ...
  \begin{mathpar}
    ...
    \begin{array}{lll}
       \theta  & \triangleq (x_i \to (\valr_i, \env_i, R_i)) & i \in
                                                               \mathbb{N}\\
      (x_i : \bang{1}\type_i), \Gamma \vDash (x_i \to (\valr_i, \env_i, R_i))
      \uplus \theta & \triangleq ~~~\tvdash{0} (\valr_i, \env_i)
                                          :  \type_i  &\conj
                                   \Gamma \vDash \theta
      \end{array}
  \end{mathpar}
...
\begin{figure}
  \begin{mathpar}
    \begin{array}{lll}
      ...
      %
      \lrv{\bang{k} \type } & = & \{  ( \valr, \env,   R) |  (\valr, \env,
                                   R ) \in \lrv{\type}  \} \\
      %
      ...
      \{( \lambda x.\expr, \env,  R) ~|~ \forall \valr', \env',
                                                             ...
                                                             \bang{k} \type_1}.\\
      & & 
   \implies   \fresh \eapp x' \land  ( \expr[x'/x], \env[x' \mapsto (\valr', \env', R' )] ) \in
          \lre{    }{ \nnatA+ k\times R' }{\type_2}     \} \\
      %
      \\
      %
...
F(\lambda x. \expr , \env,\Gamma) & = \sum_{x_i \in \fv{\lambda x.\expr} } k_i \times R_i  \\
...
\begin{thm}[TypingtoConfiguration]
  ...
  \begin{enumerate} 
   \item If $ x_1: \bang{\idx_1} \type_1, \ldots ,  x_i : \bang{\idx_i} \type_i
     ...
     R_i)   ]$, then   $
     \tvdash{\nnatA +  F( \expr, \env,  x_1: \bang{\idx_1} \type_1, \ldots ,  x_i : \bang{\idx_i} \type_i) }
     ( \expr, \env ) : \type $ .
  \end{enumerate}
\end{thm}
